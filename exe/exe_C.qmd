...
:::{.callout-note collapse=true}
#### `Germination` dataset  - Partial solution: [`Germination.R`](../code/Germination.R)
...
:::{.callout-note collapse=true}
#### `kalythos` dataset - Partial solution: [`kalythos.R`](../code/kalythos.R)
...
(a) Estimate the `LD50` (lethal dose), i.e., the age at which the probability of blindness is equal to $0.5$.  

(b) Compare the results between the logit and probit models.  

(c) ☠️ - [Optional]{.orange}. Compute an estimate for the variance of `LD50` and use it to construct a confidence interval. 
...
:::{.callout-note collapse=true}
#### `Endometrial` dataset - Partial solution: [`Endometrial.R`](../code/Endometrial.R)
...
:::{.callout-note collapse=true}
#### Exercise A - Solution: [`notes.pdf`](exe_C_A.pdf)
...
:::{.callout-note collapse=true}
#### Exercise B - Solution: [`notes.pdf`](exe_C_B.pdf)

Explain how the expression for $\text{var}(\hat{\beta})$ in logistic regression [suggest]{.orange} that the standard errors of $\hat{\beta}_j$ tend to be smaller as you obtain more data. 
