...
Suppose the logistic model holds in which $x$ is uniformly distributed on the interval $[0, 100]$ and $\text{logit}(\pi_i) = -2.0 + 0.04 x_i$. Randomly generate $100$ independent observations from this model. Plot the response residuals against $x$ and against the fitted values. Why do residual plots for binary data have this appearance?
:::

:::{.callout-note collapse=true}
#### Exercise D

Let $m_i Y_i$ be independent $\text{Binomial}(m_i, \pi_i)$ random variables for $i=1,\dots,n$. Consider the [null model]{.blue}, in which which $\pi_1 = \cdots = \pi_n$. 
