...
The data contained in the `Germination` dataframe, that can be found in the `MLGdata` library, were obtained from a $2 \times 2$ factorial experiment (two factors, each with two levels) to compare two seeds, *Orobanche aegyptiaca* 75 and *Orobanche aegyptiaca* 73, germinated on two different root extracts: bean and cucumber. The source of the data is

> Cox, D. R. e Snell, E. J. (1989). *Analysis of Binary Data*, 2nd ed. London: Chapman & Hall/CRC.

For each combination, the total number of seeds $m_i$ and the number of seeds that germinate $S_i$ are reported.  
