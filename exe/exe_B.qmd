...
i. Verify that, assuming $k$ is known, this distribution belongs to the exponential-dispersion family and identify its characteristic elements (canonical parameter, $a_i(\phi), b(\cdot), c(\cdot)$ functions, dispersion parameter, variance function).

ii. Using the properties of exponential families, recover the well-known relations  
   $$
   \mathbb{E}(Y) = \frac{k(1-\pi)}{\pi}, 
   \qquad 
   \text{var}(Y) = \frac{k(1-p)}{\pi^2}.
   $$  
