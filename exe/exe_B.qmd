...
(c) Provide a prediction for the `GAD` for a mother being exposed to a dose of $100$ (mg/L) of `DDE` and compare it with the prediction for a mother not being exposed to `DDE`. 
...
$$
Do they differ across models? [Hint]{.blue}: use the function `pnorm` if you considered a normal model and `pgamma` if you considered a gamma GLM. Be careful with the parametrization!

(e) Provide an estimate for the probability of pre-term delivery $\psi$ for a mother being exposed to a dose of $100$ (mg/L) of `DDE` and compare it with the probability for a mother not being exposed to `DDE`. 

(f) ☠️ - [Optional]{.orange}. Provide a [prediction interval]{.blue}  for the `GAD` for a mother being exposed to a dose of $100$ (mg/L) of `DDE` and compare it with the prediction for a mother not being exposed to `DDE`. 
:::
...
:::{.callout-note collapse=true}
#### Exercise A - Solution: [`notes.pdf`](exe_B_A.pdf)
...
:::{.callout-note collapse=true}
#### Exercise B - Solution: [`notes.pdf`](exe_B_B.pdf)
...
i. Verify that, assuming $k$ is known, this distribution belongs to the exponential-dispersion family and identify its characteristic elements: canonical parameter, $a_i(\cdot), b(\cdot), c(\cdot)$ functions, dispersion parameter, and variance function.
...
:::{.callout-note collapse=true}
#### Exercise C - Solution: [`notes.pdf`](exe_B_C.pdf)
...
[Hint]{.blue}: there are almost no calculations to do at points i. and ii., only substitutions. This exercise is designed to make you familiar with the notation and the general formulas. Point iii. is more elaborate and you will need to use the derivative of the Cauchy link, which is $g'(\mu) = \pi /\sin^2(\pi \mu)$.
:::

:::{.callout-note collapse=true}
#### Exercise D - Solution: [`notes.pdf`](exe_B_D.pdf)

Explicitly derive the contribution of a single observation to the deviance (i.e. $d_i$) for a Gamma generalized linear model. Then, write down the expression of the deviance $D(\hat{\bm{\mu}}; \boldsymbol{y})$ for a sample of size $n$.
:::

:::{.callout-note collapse=true}
#### Exercise E - Solution: [`notes.pdf`](exe_B_E.pdf)

Let $Y_i$ be independent random variables, with corresponding values $x_1,\dots,x_n$ of a [univariate]{.blue} quantitative covariate. Consider a GLM with [no intercept]{.blue} and $p=1$, i.e.
$$
\eta_i = \beta x_i, \qquad i=1,\dots,n,
$$
under each of the following exponential–dispersion models (mean–variance pairs):
  
1. $Y_i \sim \text{ED}(\mu_i, \phi)$ and $\mu_i \in \mathbb{R}$, with $\phi > 0$ known.  
2. $Y_i \sim \text{ED}(\mu_i, \mu_i)$ and $\mu_i \in (0, \infty)$.
3. $Y_i \sim \text{ED}(\mu_i,\mu_i(1 - \mu_i))$ and $\mu_i \in (0, 1)$.  

For each model consider the [canonical link]{.orange} and:
...
ii. Derive the score function $\ell_*(\beta)$ and the observed/expected Fisher information $I$.

iii. Obtain the maximum likelihood estimator $\hat\beta$ and provide an approximation to its sampling distribution. 

iv. For a fixed covariate value $x_i$, construct an approximate $1-\alpha$ confidence interval for the linear predictor $\eta_i = \beta x_i$. Then derive the corresponding confidence interval for $\mu_i$.  Indicate in which of the four cases these intervals are also [exact]{.blue} $1-\alpha$ intervals.
:::

:::{.callout-note collapse=true}
#### Exercise F - Solution: [`notes.pdf`](exe_B_F.pdf)
...
iii. Using the likelihood equations, show that the same result holds for any GLM of the form $g(\mu_i) = \beta_1 + \beta_2 x_i$ with a binary indicator predictor. [Note]{.orange}: if the weights $\omega_i$ are non-constant, then $\hat\mu_A$ and $\hat\mu_B$ equal the respective weighted means.
:::

:::{.callout-note collapse=true}
#### Exercise G - Solution: [`notes.pdf`](exe_B_G.pdf)
...
Then, explain why a GLM with a [canonical link]{.orange} function and constant weights $\omega_i$ requires the inclusion of an intercept term in order to ensure that this equality holds.
:::

:::{.callout-note collapse=true}
#### Exercise H - Solution: [`notes.pdf`](exe_B_H.pdf)
