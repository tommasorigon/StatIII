...
The `Crabs` dataset is [available here](../data/Crabs.dat) and come from a study of female horseshoe crabs[^1] on an island in the Gulf of Mexico. 
...
satellites. Explanatory variables are the female crab's color (`color`), spine condition (`spine`), weight (`weight`), and carapace width (`width`). The source of the dataset is the textbook:
