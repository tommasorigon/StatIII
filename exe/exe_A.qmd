...
Prove that the variance-stabilizing transform for gamma random variables $Y_i\sim \text{Gamma}(\alpha, \beta_i)$ with mean $\mu_i = \alpha/\beta_i$ and variance $\alpha/\beta_i^2$, is $g(Y_i) = \log(Y_i)$. 

[Hint]{.blue}: use the fact that if $Y \sim \text{Gamma}(\alpha, \beta)$ with mean $\alpha/\beta$ and variance $\alpha/\beta^2$, then for large values of $\alpha$ we have the approximation
$$
Y \; \dot{\sim}\;\text{N}(\alpha/\beta, \alpha/\beta^2). 
$$
:::

:::{.callout-note collapse=true}
#### Agresti (2015), Exercise 2.4
...
:::{.callout-note collapse=true}
#### `Abrasion` dataset. Salvan et al. (2020), Exercise 1.12
...
::: {.callout-note collapse=true}
#### `FEV` dataset. Agresti (2015), Exercise 1.21
...
[Download the data](../data/FEV.dat) and fit linear models for $y$ using the following sets of predictors: (i) $x_1$ only, (ii) $x_2$ only, (iii) both $x_1$ and $x_2$. Test to see whether the interaction terms are needed. For each model, [interpret]{.blue} the estimated parameters. Check the underlying assumptions of the fitted models.
...
:::{.callout-note collapse=true}
#### `Anorexia` dataset. Agresti (2015), Exercise 1.24
...
[Download the data](../data/Anorexia.dat) and fit a linear model where the outcome is the [post-treatment weight]{.blue} and the predictors are the [baseline weight]{.orange} and the [therapy group]{.orange}. [Interpret]{.blue} the estimated parameters of the model. Check the underlying assumptions.
