...
- We suppose the errors $\bm{\epsilon}$ have zero mean $\mathbb{E}(\bm{\epsilon}) = 0$ and are [uncorrelated]{.orange} $\text{var}(\bm{\epsilon}) = \sigma^2I_n$.

- Then, the estimator $\hat{\beta}$ is [unbiased]{.orange} $\mathbb{E}(\hat{\beta}) = \beta$ and its [variance]{.blue} is $\text{var}(\hat{\beta}) = \sigma^2 (\bm{X}^T\bm{X})^{-1}$. Since $\sigma^2$ is also unknown, we can estimate the variances of $\hat{\beta}$ using $$
...
- The diagonal elements $h_{ii} \in [0, 1]$ of the matrix $\bm{H}$ are called [leverages]{.blue} and it holds
$$
\text{var}(\hat{Y}_i) = \sigma^2 h_{ii}, \qquad \text{var}(Y_i - \hat{Y}_i) = \sigma^2 (1 - h_{ii}), \qquad \text{cor}(Y_i, \hat{Y}_i) = \sqrt{h_{ii}}.
$$
The leverage $h_{ii}$ determines the [precision]{.orange} with which $\hat{Y}_i$ predicts $Y_i$. For large $h_{ii}$ close to $1$, $\text{cor}(Y_i, \hat{Y}_i) \approx 1$, therefore changes of a single point $Y_i$ leads to significant changes in $\hat{Y}_i$.

- Leverages also appear in the definition of [standardized residuals]{.blue}:
$$
\tilde{r}_i = \frac{r_i}{\sqrt{s^2(1 - h_{ii})}} = \frac{y_i - \bm{x}_i^T\hat{\beta}}{\sqrt{s^2(1 - h_{ii})}},
$$
where $r_i = y_i - \bm{x}_i^T\hat{\beta}$ are the [residuals]{.orange}.
...
$$
p \cdot d_i = (\hat{\beta} - \hat{\beta}_{-i})^T\widehat{\text{var}}(\hat{\beta})^{-1}(\hat{\beta} - \hat{\beta}_{-i}) = \tilde{r}_i^2 \frac{h_{ii}}{1 - h_{ii}}.
$$
...
- The density of the [original data]{.orange} is $f_Y(\bm{y}) = f(\bm{y}(\lambda))\prod_{i=1}^ny_i^{\lambda - 1}$ after accounting for the [Jacobian]{.orange} of the transformation. The [log-likelihood]{.blue} therefore is
$$
