...
-   In matrix notation, the predicted values can be obtained as $$
    ...
    $$
where $\boldsymbol{H}$ is a $n \times n$ [projection matrix]{.orange} matrix
...
$$
The leverage $h_i$ determines the [precision]{.orange} with which $\hat{Y}_i$ predicts $Y_i$. For large $h_i$ close to $1$, $\text{cor}(Y_i, \hat{Y}_i) \approx 1$, therefore changes of a single point $Y_i$ leads to significant changes in $\hat{Y}_i$.
...
- An observation is [influent]{.blue} if it has high leverage and high squared residual. [Cook’s distance]{.orange} $c_i$ is based on the change in $\hat{\beta}$ when the observation is removed:
$$
...
-   However, the plot of the [residuals]{.orange}
    $r_i = y_i - \hat{y}_i$ suggests that the homoschedasticity
...
- **(A.2)** [Homoschedasticity]{.orange} and [uncorrelation]{.orange} of the errors, namely $\text{var}(\boldsymbol{\epsilon}) = \sigma^2 I_n$.
...
![](img/plane.jpg){.nostretch fig-align="center" width="500px"}
...
- Suppose now that the linearity assumption **(A.1)** is valid but [homoschedasticity]{.orange} of the errors **(A.2)** is [not]{.orange}. Instead, we consider [heteroschedastic errors]{.blue}:
$$
...
:::callout-tip
The OLS estimator in presence of heteroschedasticity still gives a [good point estimate]{.blue}. However, the OLS estimator is [not efficient]{.orange} and the classical [standard errors]{.orange} are [wrong]{.orange}. 
:::
...
- Let us consider again the case of [heteroschedastic errors]{.blue}:
$$
...
$$
However, the data are  [heteroschedastic]{.orange}, because $\text{var}(Y_i) = m_i \pi_i(1- \pi_i)$.
