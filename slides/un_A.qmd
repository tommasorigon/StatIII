...
## Influence point (outlier and leverage)
...
:::callout-tip
Note that Box and Cox themselves suggested using this approach as an [exploratory tool]{.blue}. For instance, the optimal value $\hat{\lambda} = 0.4210283$ may be [hard to interpret]{.orange} but it could indicate that a square root transformation is appropriate.
:::
...
:::callout-tip
Under **(A.1)**-**(A.2)**, even without requring normality of the errors **(A.3)**, we obtain the usual formulas:
$$
...
$$
If in addition we assume Gaussianity of the errors, that is $\bm{\epsilon} \sim \text{N}_n(0, \sigma^2\bm{W})$, then
$$
...
[^white]: White originally proposed the simpler version $\hat{w}_i = r_i^2$. Another variant is $\hat{w}_i = r_i^2 / (1 - h_i)^2$. 
...
:::callout-tip
Suppose $Y_i \sim \text{Binomial}(\pi, n_i)$. The variance stabilizing transformation is not fully satisfactory:

  - It [complicates]{.orange} the [interpretation]{.orange}, because it models $\mathbb{E}\{g(Y_i)\}$ instead of $\mathbb{E}(Y_i)$;
  - It is an [asymptotic approximation]{.blue}, and is only valid for $n_i \rightarrow \infty$.

Besides, this transform is clearly not applicable when $n_i = 1$ and $Y_i \in \{0, 1\}$, a very common problem called [binary regression]{.orange}.
:::
