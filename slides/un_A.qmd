...
$$
where $\bm{\Omega} = \text{diag}(\omega_1,\dots,\omega_n)$ are positive [weights]{.orange}. However, here we assume that the weights $\omega_1,\dots,\omega_n$ are [known]{.blue}, a common situation in survey design. 
...
$$
Moreover, if $\bm{\epsilon} \sim \text{N}_n(0, \sigma^2\bm{\Omega}^{-1})$ it also coincides with the [maximum likelihood]{.orange} estimator. 
