...
-   This unit will cover the following [topics]{.orange}:
    ...
    - [Recap]{.blue}: linear models and the modeling process
...
:::callout-tip
The content of this Unit is covered in [Chapter 1]{.orange} of @Salvan2020. Alternatively, see [Chapter 2]{.blue} of @Agresti2015 or [Chapter 5]{.grey} of @Azzalini2008.
:::
...
-   A [general]{.orange} and [more flexible formulation]{.orange} for
    ...
    covariates]{.blue}
    $\boldsymbol{x}_i = (x_{i1},\dots,x_{ip})^T \in \mathbb{R}^p$ and a random
...
-   To estimate the unknown parameters $\beta$, a possibility is to rely
    ...
    $\boldsymbol{x}_i = (x_{i1},\dots,x_{ip})^T$ and the observed realizations
...
-   The [predicted values]{.blue} are
    $\hat{y}_i = \widehat{\mathbb{E}(Y_i)} = f(\boldsymbol{x}_i; \hat{\beta})$, for
    $i=1,\dots,n.$
:::
...
covariates through the function$$
    ...
    $$ where $\boldsymbol{x}_i = (x_{i1},\dots,x_{ip})^T$ is a vector of
...
-   The [response random variables]{.orange} are collected in the random
    vector $\boldsymbol{Y} = (Y_1,\dots,Y_n)^T$, whose [observed
    realization]{.blue} is $\boldsymbol{y} = (y_1,\dots,y_n)^T$.

-   The [design matrix]{.blue} is an $n \times p$ matrix, comprising the
    covariate values, defined by $$
    \boldsymbol{X} = 
    \begin{bmatrix} 
    x_{11} & \cdots & x_{1p}\\
    \vdots & \ddots & \vdots \\
...
-   The $j$th variable (column) is denoted with $\tilde{\boldsymbol{x}}_j$,
    whereas the $i$th observation (row) is $\boldsymbol{x}_i$: $$
...
-   Then, a [linear model]{.blue} can be written using the [compact
    ...
    $$ where $\boldsymbol{\epsilon} = (\epsilon_1,\dots,\epsilon_n)^T$ is a
...
-   The optimal set of coefficients $\hat{\beta}$ is the minimizer of
    the [least squares criterion]{.orange} $$
    ...
    ||\boldsymbol{y}|| = \sqrt{y_1^2 + \cdots + y_n^2},$$ denotes the [Euclidean
    norm]{.blue}.

. . .

:::callout-note
#### Least squares estimate (OLS)

If the design matrix has [full rank]{.blue}, that is, if
$\text{rk}(\boldsymbol{X}^T\boldsymbol{X}) = p$, then the [least squares
...
-   In matrix notation, the predicted values can be obtained as $$
    ...
    $$
where $\boldsymbol{H}$ is an $n \times n$ [projection matrix]{.orange}
    sometimes called [hat matrix]{.blue}. The matrix is symmetric and idempotent, meaning that $\boldsymbol{H} = \boldsymbol{H}^T$ and $\boldsymbol{H}^2 = \boldsymbol{H}$.
...
- Recall that the errors $\boldsymbol{\epsilon}$ have zero mean $\mathbb{E}(\boldsymbol{\epsilon}) = 0$ and are [uncorrelated]{.orange} $\text{var}(\boldsymbol{\epsilon}) = \sigma^2I_n$.

- Then, the estimator $\hat{\beta}$ is [unbiased]{.orange} $\mathbb{E}(\hat{\beta}) = \beta$ and its [variance]{.blue} is $\text{var}(\hat{\beta}) = \sigma^2 (\boldsymbol{X}^T\boldsymbol{X})^{-1}$. Since $\sigma^2$ is also unknown, we can estimate the variances of $\hat{\beta}$ as follows: $$
...
-   Confidence intervals and Wald tests can be obtained through
...
- The diagonal elements $h_i \in [0, 1]$ of the matrix $\boldsymbol{H}$ are called [leverages]{.blue} and it holds
$$
\text{var}(\hat{Y}_i) = \sigma^2 h_i, \qquad \text{var}(Y_i - \hat{Y}_i) = \sigma^2 (1 - h_i), \qquad \text{cor}(Y_i, \hat{Y}_i) = \sqrt{h_i}.
$$
The leverage $h_i$ determines the [precision]{.orange} with which $\hat{Y}_i$ predicts $Y_i$. For large $h_i$ close to $1$, $\text{cor}(Y_i, \hat{Y}_i) \approx 1$, therefore a change in a single point $Y_i$ leads to significant changes in $\hat{Y}_i$.

- Leverages also appear in the definition of [standardized residuals]{.blue}:
$$
\tilde{r}_i = \frac{r_i}{\sqrt{s^2(1 - h_i)}} = \frac{y_i - \boldsymbol{x}_i^T\hat{\beta}}{\sqrt{s^2(1 - h_i)}},
$$
where $r_i = y_i - \boldsymbol{x}_i^T\hat{\beta}$ are the (raw) [residuals]{.orange}.

. . .
 
- An observation is [influential]{.blue} if it has high leverage and high squared residual. [Cook’s distance]{.orange} $c_i$ is based on the change in $\hat{\beta}$ when the observation is removed:
$$
c_i = \frac{1}{p}(\hat{\beta} - \hat{\beta}_{-i})^T\widehat{\text{var}}(\hat{\beta})^{-1}(\hat{\beta} - \hat{\beta}_{-i}) = \tilde{r}_i^2 \frac{h_i}{p(1 - h_i)}.
$$
Cook's distance is considered relatively large when $c_i \ge 1$. 
...
- [Left plot]{.orange}: leverage, not outlier. [Central plot]{.blue}: outlier, not leverage. [Right plot]{.grey}: influence point = leverage + outlier.
...
-   We obtain the following [summary]{.orange} for the regression coefficients $\hat{\beta}$.
...
-   Moreover, the coefficient of determination $R^2$, the residual
    standard deviation $s$, and the deviance $D(\hat{\beta})$ are:
...
## A first model: comments and criticisms

-   Is this a good model?

-   The overall fit [seems satisfactory]{.blue} at first glance,
    especially if we aim at predicting the urban distance of cars with
...
## A second model: graphical diagnostics
...
## A second model: comments and criticisms
...
-   Do not mix [apples]{.blue} and [oranges]{.orange}! Compare $R^2$s
...
-   Yet, there is still some heteroscedasticity in the residuals --- is
    this due to a missing covariate that has not been included in the
    model?
...
## A third model: comments and criticisms
...
-   The estimates are coherent with our expectations, based on common knowledge. Have a look at the book (@Azzalini2012) for a detailed explanation of $\beta_4$!
...
- **(A.1)** [Linear structure]{.blue}, namely $\boldsymbol{Y} = \boldsymbol{X}\beta + \boldsymbol{\epsilon}$ with $\mathbb{E}(\boldsymbol{\epsilon}) = 0$, implying $\mathbb{E}(\boldsymbol{Y}) = \boldsymbol{X}\beta$. [^err1] 

- **(A.2)** [Homoscedasticity]{.orange} and [lack of correlation]{.orange} of the errors, namely $\text{var}(\boldsymbol{\epsilon}) = \sigma^2 I_n$.

- **(A.3)** [Gaussianity]{.grey}, namely $\boldsymbol{\epsilon} \sim \text{N}_n(0, \sigma^2 I_n)$. In other words, the errors $\epsilon_i \overset{\text{iid}}{\sim}N(0, \sigma^2)$ are iid Gaussian random variables with zero mean and variance $\sigma^2$.

It is also commonly assumed that $\text{rk}(\boldsymbol{X}) = p$, otherwise the model is not identifiable.
:::
...
<!-- - Even if the data are [heteroscedastic]{.orange} the OLS estimator is still a very reasonable choice, but we need to "correct" the standard errors to account for that.  -->

[^err1]: The assumption $\mathbb{E}(\boldsymbol{\epsilon}) = 0$ is not restrictive when the intercept is included in $\boldsymbol{X}$: a non-zero mean of the errors would simply be absorbed by $\beta_1$.


## Robust estimation and assumptions

![](img/plane.jpg){.nostretch fig-align="center" width="50%"}
...
- Let us consider the case in which assumptions **(A.1)**-**(A.2)** are [valid]{.blue} but **(A.3)** [is not]{.orange}, that is $\mathbb{E}(\boldsymbol{\epsilon}) = 0$ and $\text{var}(\boldsymbol{\epsilon}) = \sigma^2 I_n$, but $\epsilon$ does [not]{.orange} follow [a Gaussian]{.orange} distribution. 
...
- The OLS estimate $\hat{\beta}$ is [no longer]{.orange} the [maximum likelihood]{.orange} estimator, but it [preserves]{.blue} most of its [properties]{.blue} and a [geometric interpretation]{.blue}. 
...
$$
Moreover, because of [Gauss-Markov]{.blue} theorem, the OLS estimator $\hat{\beta}$ is the most [efficient]{.orange} within the class of linear and unbiased estimators ([BLUE]{.blue}) for any distribution of the errors $\boldsymbol{\epsilon}$. 
:::


- In fact, note that the [proof]{.orange} of the Gauss-Markov theorem requires **(A.1)**-**(A.2)** but [not]{.orange} **(A.3)**.


## Non-normality of the errors II

- When the errors are non-Gaussian the [exact inferential results]{.blue} are not valid. In particular $\hat{\beta}$ no longer follows a Gaussian distribution.

- However, a [central limit theorem]{.orange} can be invoked under very mild conditions on the design matrix $\boldsymbol{X}$.
...
## Heteroscedasticity of the errors I 📖

- Suppose now that the linearity assumption **(A.1)** is valid but [homoscedasticity]{.orange} of the errors **(A.2)** is [not]{.orange}. Instead, we consider [heteroscedastic errors]{.blue}:
$$
\text{var}(\boldsymbol{\epsilon}) = \boldsymbol{\Sigma},\quad \text{or equivalently} \quad \text{var}(Y_i) = \sigma^2_i, \quad i=1,\dots,n
$$
where $\boldsymbol{\Sigma} = \text{diag}(\sigma^2_1,\dots,\sigma_n^2)$ is a diagonal matrix with positive entries. 

- The OLS estimator is still [unbiased]{.orange}, with a [modified covariance]{.blue} structure[^diagonal] 
$$
...
$$
Under suitable but mild conditions on $\boldsymbol{X}$ and $\boldsymbol{\Sigma}$, the estimator is also [consistent]{.orange}. 


[^diagonal]: These results are valid even when the matrix $\boldsymbol{\Sigma}$ is non-diagonal. This is useful to model correlated responses. 

## Heteroscedasticity of the errors II

:::callout-tip
The OLS estimator in presence of heteroscedasticity still gives a [good point estimate]{.blue}. However, the OLS estimator is [not efficient]{.orange} and the classical [standard errors]{.orange} are [wrong]{.orange}. 
:::

- A potential approach is to [accept the inefficiency]{.orange} of the OLS estimator in this scenario and [correct]{.blue} the standard errors.

- The elements of $\boldsymbol{\Sigma}$ are [unknown]{.orange}, but we can estimate them from the data. Note that
$$
\text{var}(r_i) = \text{var}(y_i - \boldsymbol{x}_i^T\hat{\beta}) \approx \sigma^2_i(1 - h_i),
$$
suggesting the [estimate]{.blue} $\hat{\sigma}^2_i = r_i^2 / (1 - h_i)$. 

- This leads to the so-called [sandwich estimator]{.orange} of the covariance matrix:
$$
...
$$
where $\hat{\boldsymbol{\Sigma}} = \text{diag}(\hat{\sigma}^2_1,\dots,\hat{\sigma}^2_n)$, using the above estimates $\hat{\sigma}^2_i = r_i^2 / (1 - h_i)$. 

- These are known as [White's]{.blue} heteroscedasticity-consistent [standard errors]{.blue}. [^white]

[^white]: White originally proposed the simpler version $\hat{\sigma}^2_i = r_i^2$. Another variant is $\hat{\sigma}^2_i = r_i^2 / (1 - h_i)^2$. 

## Weighted least squares I 📖

- Let us consider again the case of [heteroscedastic errors]{.blue}:
$$
\text{var}(\boldsymbol{\epsilon}) = \sigma^2\boldsymbol{\Omega}^{-1},\quad \text{or equivalently} \quad \text{var}(Y_i) = \sigma^2_i = \frac{\sigma^2}{\omega_i}, \quad i=1,\dots,n
$$
where $\boldsymbol{\Omega} = \text{diag}(\omega_1,\dots,\omega_n)$ are positive [weights]{.orange}. However, here we assume that the weights $\omega_1,\dots,\omega_n$ are [known]{.blue}, a common situation in survey design. 

- Let us define the [standardized]{.orange} quantities:
$$
\boldsymbol{Y}^* = \boldsymbol{\Omega}^{1/2}\boldsymbol{Y}, \qquad \boldsymbol{X}^* = \boldsymbol{\Omega}^{1/2}\boldsymbol{X}. 
$$
This is equivalent to saying that $Y_i^* = \sqrt{\omega_i} Y_i$ and $x_{ij}^* = \sqrt{\omega_i} x_{ij}$. Then, it is easy to show that
$$
...
$$
namely the [assumptions]{.orange} **(A.1)** and **(A.2)** are valid in the [transformed scale]{.blue}.

- In other words, [after]{.orange} a suitable [transformation]{.orange}, we have reduced the problem to a [standard linear model]{.blue}. 

## Weighted least squares II 📖

- Thus an estimator for $\beta$, based on the transformed data, is obtained by minimizing the deviance
$$
\begin{aligned}
...
\end{aligned}
$$
which is a [weighted]{.orange} version of the original [quadratic loss]{.blue}, with [high weight = low variance]{.orange}.

- The resulting estimate minimizing $D_\text{wls}(\beta)$ in the transformed and original scales is
$$
...
$$
and it is referred to as [weighted least squares]{.blue} estimator of $\beta$.

- Such an estimator is [unbiased]{.orange} and [efficient]{.orange} ([BLUE]{.blue}), with
$$
\mathbb{E}(\hat{\beta}_\text{wls}) = \beta, \qquad \text{var}(\hat{\beta}_\text{wls}) = \sigma^2 (\boldsymbol{X}^T\boldsymbol{\Omega}\boldsymbol{X})^{-1}.
$$
Moreover, if $\boldsymbol{\epsilon} \sim \text{N}_n(0, \sigma^2\boldsymbol{\Omega}^{-1})$ it also coincides with the [maximum likelihood]{.orange} estimator. 


## Variable transformations

- Another remedy for [misspecification]{.orange} was already applied in the analysis of the car dataset, namely through [variable transformation]{.blue}.
...
If the data $y_i$ are [positive]{.orange}, we may consider a [parametric class]{.blue} of transformations:
$$
...
- We estimate $\lambda$ from the data using [maximum likelihood]{.orange}, so that the data themselves can inform us about the best transformation. We assume
$$
...
- By assumption, the distribution of the [transformed data]{.blue} $\boldsymbol{Z}_\lambda  = (g_\lambda(Y_1), \dots,g_\lambda(Y_n))^T$ is Gaussian, therefore their joint density is
$$
...
- Using standard tools of probability theory, we can obtain the density of the [original data]{.orange}: 
$$
f_Y(\boldsymbol{y})= f_Z(g_\lambda(y_1),\dots,g_\lambda(y_n))\prod_{i=1}^n\left|\frac{\partial g_\lambda(y_i)}{\partial y_i}\right|, \quad \text{where}\quad \left|\frac{\partial g_\lambda(y_i)}{\partial y_i}\right| = y_i^{\lambda - 1}.$$
The additional term is the determinant of the [Jacobian]{.orange} of the transformation. 
...
- We can [plug-in]{.orange} the above estimates into the log-likelihood. This gives the [profile log-likelihood]{.blue} for $\lambda$, which admits a very simple expression:
$$
...
:::callout-tip
Box and Cox suggested using this approach as an [exploratory tool]{.blue}. For instance, an optimal value $\hat{\lambda} = 0.4210283$ is [hard to interpret]{.orange} but it could suggest a square root transformation.
:::
...
$$
is [asymptotically Gaussian]{.blue} for large values of $\mu_i$. However, data are [heteroscedastic]{.orange}. 
...
- Let $Y_i \sim \text{Binomial}(m_i, \pi_i)$, with [trials]{.blue} $m_i$ and [success probability]{.blue} $\pi_i = f(\boldsymbol{x}_i; \beta)$. For large values of $m_i$, the [Gaussian approximation]{.orange} holds 
$$
Y_i \,\dot{\sim}\, \text{N}(m_i \pi_i, m_i\pi_i(1 - \pi_i)).
$$
However, the data are  [heteroscedastic]{.orange}, because $\text{var}(Y_i) = m_i \pi_i(1- \pi_i)$.
...
$$
Thus, the coefficient $\beta_j$ can [no longer]{.orange} be [interpreted]{.orange} as the change in the mean of $Y_i$ corresponding to a [one-unit increase]{.blue} $x_{ij} \rightarrow x_{ij}+1$ of the $j$th covariate.

. . .

- In the case of transformations of the [response]{.orange} variable we let $\mathbb{E}(g(Y_i)) = \boldsymbol{x}_i^T\beta$. However:  
$$
  g(\mathbb{E}(Y_i)) \neq \mathbb{E}(g(Y_i)) \quad \Longrightarrow \quad \mathbb{E}(Y_i) \neq g^{-1}(\boldsymbol{x}_i^T\beta).
$$
Thus $\hat{y}_i = g^{-1}(\boldsymbol{x}_i^T\hat{\beta})$ is a [reasonable prediction]{.blue} for $Y_i$ and [not an estimate]{.orange} for its [mean]{.orange}.
...
:::callout-tip
Suppose $Y_i \sim \text{Binomial}(m_i, \pi_i)$. The variance stabilizing transformation is not fully satisfactory:

  - It [complicates]{.orange} the [interpretation]{.orange}, because it models $\mathbb{E}\{g(Y_i)\}$ instead of $\mathbb{E}(Y_i)$;
  - It is an [asymptotic approximation]{.blue}, and is only valid for $m_i \rightarrow \infty$.
...
Besides, this transform is clearly not applicable when $m_i = 1$ and $Y_i \in \{0, 1\}$, a very common problem called [binary regression]{.orange}.
:::

- If we know that $Y_i$ follows, say, a Bernoulli or a Gamma distribution, then we should use the [appropriate likelihood]{.blue} rather than a [Gaussian approximation]{.orange}. 

- [Generalized Linear Models]{.blue} provide a [much more elegant solution]{.orange} to the above problem. 
