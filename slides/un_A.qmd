...
- The above assumptions are often taken a little [too seriously]{.blue}. If one of them is violated, it is not necessarily a huge problem, because
...
- In other words, while the model may have been incorrectly specified for the original data, it could become [appropriate]{.blue} once the [transformations]{.orange} are considered, namely
$$
...
- However, this is not very problematic in practice because a [central limit theorem]{.orange} can be invoked under very mild conditions on the design matrix $\bm{X}$.
...
:::callout-tip
[Dropping normality]{.blue} of the errors is [not a major concern]{.orange}: the OLS estimator preserves most of its properties, including approximate normality for sufficiently large $n$.

In undergraduate courses there is often an [over-emphasis]{.orange} on testing whether the residuals are Gaussian. This is likely an [overly conservative]{.orange} approach: even if normality is rejected, the practical implications are minimal.
:::
