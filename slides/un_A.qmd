...
- We assume that the errors $\bm{\epsilon}$ have zero mean $\mathbb{E}(\bm{\epsilon}) = 0$ and are [uncorrelated]{.orange} $\text{var}(\bm{\epsilon}) = \sigma^2I_n$.
...
$$
\beta \sim \text{N}_p(\beta, (\bm{X}^T\bm{X})^{-1}\bm{X}^T\bm{\Sigma}\bm{X} (\bm{X}^T\bm{X})^{-1}).
$$
