...
-   In matrix notation, the predicted values can be obtained as $$
    ...
    $$
where $\bm{H}$ is a $n \times n$ [projection matrix]{.orange} matrix
    sometimes called [hat matrix]{.blue}. The matrix is idempotent, meaning that $\bm{H} = \bm{H}^T$ and $\bm{H}^2 = \bm{H}$.
...
- Recall that the errors $\bm{\epsilon}$ have zero mean $\mathbb{E}(\bm{\epsilon}) = 0$ and are [uncorrelated]{.orange} $\text{var}(\bm{\epsilon}) = \sigma^2I_n$.

- Then, the estimator $\hat{\beta}$ is [unbiased]{.orange} $\mathbb{E}(\hat{\beta}) = \beta$ and its [variance]{.blue} is $\text{var}(\hat{\beta}) = \sigma^2 (\bm{X}^T\bm{X})^{-1}$. Since $\sigma^2$ is also unknown, we can estimate the variances of $\hat{\beta}$ as follows: $$
...
- The diagonal elements $h_i \in [0, 1]$ of the matrix $\bm{H}$ are called [leverages]{.blue} and it holds
$$
...
$$
where $r_i = y_i - \bm{x}_i^T\hat{\beta}$ are the (raw) [residuals]{.orange}.

. . .
 
- An observation is [influent]{.blue} if it has high leverage and high squared residual. [Cook’s distance]{.orange} $d_i$ is based on the change in $\hat{\beta}$ when the observation is removed:
$$
p \cdot d_i = (\hat{\beta} - \hat{\beta}_{-i})^T\widehat{\text{var}}(\hat{\beta})^{-1}(\hat{\beta} - \hat{\beta}_{-i}) = \tilde{r}_i^2 \frac{h_i}{1 - h_i}.
$$
Cook's distance is considered relatively large when $d_i \ge 1$. 
...
$$
Moreover, because of [Gauss-Markov]{.blue} theorem, the OLS estimator $\hat{\beta}$ is the most [efficient]{.orange} within the class of linear and unbiased estimators ([BLUE]{.blue}) for any distribution of the errors $\bm{\epsilon}$. 
:::
...
$$
where $\bm{\Sigma} = \text{diag}(\sigma^2_1,\dots,\sigma_n^2)$ is a diagonal matrix with positive entries. 
...
$$
If in addition we assume Gaussianity of the errors, that is $\bm{\epsilon} \sim \text{N}_n(0,\bm{\Sigma})$, then
$$
\hat{\beta} \sim \text{N}_p(\beta, (\bm{X}^T\bm{X})^{-1}\bm{X}^T\bm{\Sigma}\bm{X} (\bm{X}^T\bm{X})^{-1}).
$$
Under suitable but mild conditions on $\bm{X}$ and $\bm{\Sigma}$, the estimator is also [consistent]{.orange}. 


[^diagonal]: These results are valid even when the matrix $\bm{\Sigma}$ is non-diagonal. This is useful to model correlated responses. 
...
- The elements of $\bm{\Sigma}$ are [unknown]{.orange}, but we can estimate them from the data. Note that
$$
...
$$
where $\hat{\bm{\Sigma}} = \text{diag}(\hat{w}_1,\dots,\hat{w}_n)$ and $\hat{w}_i = r_i^2 / (1 - h_i)$. 
...
$$
\text{var}(\bm{\epsilon}) = \sigma^2\bm{W}^{-1},\quad \text{or equivalenty} \quad \text{var}(Y_i) = \sigma^2_i = \frac{\sigma^2}{w_i}, \quad i=1,\dots,n
$$
where $\bm{W} = \text{diag}(w_1,\dots,w_n)$ are positive [weights]{.orange}. However, here we assume that the weights $w_1,\dots,w_n$ are [known]{.blue}, a common situation in survey design. 

- Let us define the [standardized]{.orange} quantities:
$$
\bm{Y}^* = \bm{W}^{1/2}\bm{Y}, \qquad \bm{X}^* = \bm{W}^{1/2}\bm{X}. 
$$
This is equivalent to say that $Y_i^* = \sqrt{w_i} Y_i$ and $x_{ij}^* = \sqrt{w_i} x_{ij}$. Then, it is easy to show that
$$
\mathbb{E}(\bm{Y}^*) = \bm{X}^*\beta, \qquad \text{var}(\bm{Y}^*) = \sigma^2\bm{W}^{1/2}\bm{W}^{-1}\bm{W}^{1/2}= \sigma^2 I_n,
$$
...
$$
\begin{aligned}
D_\text{wls}(\beta) &= (\bm{y}^* - \bm{X}^*\beta)^T(\bm{y}^* - \bm{X}^*\beta) = (\bm{y} - \bm{X}\beta)^T\bm{W}(\bm{y} - \bm{X}\beta) \\
&=\sum_{i=1}^n w_i (y_i - \bm{x}_i^T\beta)^2.
\end{aligned}
$$
...
$$
\hat{\beta}_\text{wls} = [(\bm{X}^*)^T\bm{X}^*]^{-1}(\bm{X}^*)^T\bm{y}^* = (\bm{X}^T\bm{W}\bm{X})^{-1}\bm{X}^T\bm{W}\bm{y} 
$$
...
$$
\mathbb{E}(\hat{\beta}_\text{wls}) = \beta, \qquad \text{var}(\hat{\beta}_\text{wls}) = \sigma^2 (\bm{X}^T\bm{W}\bm{X})^{-1}.
$$
Moreover, if $\bm{\epsilon} \sim \text{N}_n(0, \sigma^2\bm{W}^{-1})$ it also coincides with the [maximum likelihood]{.orange} estimator. 
...
- By assumption, the distribution of the [transformed data]{.blue} $\bm{Z}_\lambda  = (g_\lambda(Y_1), \dots,g_\lambda(Y_n))^T$ is Gaussian, therefore their joint density is
$$
...
$$
f_Y(\bm{y})= f_Z(g_\lambda(y_1),\dots,g_\lambda(y_n))\prod_{i=1}^n\left|\frac{\partial g_\lambda(y_i)}{\partial y_i}\right|, \quad \text{where}\quad \left|\frac{\partial g_\lambda(y_i)}{\partial y_i}\right| = y_i^{\lambda - 1}.$$
...
$$
\hat{\beta}(\lambda) = (\bm{X}^T\bm{X})^{-1}\bm{X}^T\bm{z}_\lambda, \qquad \hat{\sigma}^2(\lambda) = \frac{1}{n}(\bm{z}_\lambda - \bm{X}\hat{\beta}(\lambda))^T(\bm{z}_\lambda - \bm{X}\hat{\beta}(\lambda)),
$$
...
- Let $Y_i \sim \text{Binomial}(\pi_i, m_i)$, with [success probability]{.blue} $\pi_i = f(\bm{x}_i; \beta)$ and [trials]{.blue} $m_i$. For large values of $m_i$, the [Gaussian approximation]{.orange} holds 
$$
...
Suppose $Y_i \sim \text{Binomial}(\pi, m_i)$. The variance stabilizing transformation is not fully satisfactory:
  ...
  - The transform depends on $m_i$, therefore we cannot make predictions for a generic covariate value $\bm{x}_i$ without knowing the associated $m_i$. 
