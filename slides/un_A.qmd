...
$\text{rk}(\boldsymbol{X}^T\boldsymbol{X}) = p$, then the [least square
...
-   In matrix notation, the predicted values can be obtained as $$
    ...
    $$
where $\boldsymbol{H}$ is a $n \times n$ [projection matrix]{.orange}
...
- **(A.2)** [Homoscedasticity]{.orange} and [uncorrelation]{.orange} of the errors, namely $\text{var}(\boldsymbol{\epsilon}) = \sigma^2 I_n$.
...
It is also commonly asked that $\text{rk}(\boldsymbol{X}) = p$, otherwise the model is not identifiable.
:::
...
- In other words, [after]{.orange} a suitable [transformation]{.orange}, we reconducted the problem to a [standard linear model]{.blue}. 
