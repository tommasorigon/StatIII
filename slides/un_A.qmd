...
- Leverages appear also in the definition of [standardized residuals]{.orange}:
$$
...
- An observation is [influent]{.blue} if it has high leverage and residual. [Cook’s distance]{.orange} $d_i$ is based on the change in $\hat{\beta}$ when the observation is removed:
$$
p \cdot d_i = (\hat{\beta} - \hat{\beta}_{-i})^T\widehat{\text{var}}(\hat{\beta})^{-1}(\hat{\beta} - \hat{\beta}_{-i}) = \tilde{r}_i^2 \frac{h_{ii}}{1 - h_{ii}}.
$$
The Cook's distance is considered relatively large when $d_i \ge 1$. 
