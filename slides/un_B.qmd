...
- These indices are [difficult to interpret]{.orange} and often mislead those accustomed with standard $R^2.$ A pseudo-$R^2 \approx 0.4$ may indicate a nearly perfect fit (i.e. `Beetles` data), which is confusing. 

- On top of this, these pseudo-$R^2$ produce different answers depending on the [aggregation]{.blue} of the data. 

. . .

- The recommendation is to [avoid pseudo-$R^2$]{.orange} and rely instead on indices tailored for the data at hand, such as the ROC curve for binary data, or the correlation between $\bm{y}$ and $\hat{\bm{\mu}}$.
...
- The [deviance]{.blue} equals $D(\bm{y}; \hat{\bm{\mu}}) = 30.203$, with $n - p = 14 - 2 = 12$ degrees of freedom. The observed $X^2$ [Pearson statistic]{.orange} equals $30.203$ (they coincide!)

- Using the $X^2$ statistic with $12$ degrees of freedom, we obtain the p-value $\mathbb{P}(X^2 > 30.204) = 0.0026$, therefore [rejecting]{.orange} the hypothesis that this model has a comparable fit with the saturated model.
...
- A few remedies could be:
  ...
  b. Including an [additional covariate]{.blue} (if available), and/or considering non-linear [trasformations]{.orange} of the available ones;
...
- It turns out that selecting the link function $g(\mu_i) = \sqrt{\mu_i}$ yields a much better fit with $X^2 = 17.968$, whose p-value is $0.12$. This is not the only possible solution. 
