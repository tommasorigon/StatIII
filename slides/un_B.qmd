...
\end{aligned}
$$
for $y_i \in \{0, 1/m_i, 2/m_i, \dots, m_i/m_i\}$. This can be written as
$$
...
Moreover, the matrices $\bm{I}$ and $\bm{I}^{-1}$ are sufficient for inference on $\beta$ and there is no need to compute $i_{\phi \phi}$. Moreover, the maximum likelihood $\hat{\beta}$ can also be computed without knowing $\phi$. 
:::
...
- In a classical [Gaussian linear model]{.blue} the weight matrix is $\bm{W} = \sigma^2 I_n$, therefore
$$
...
- In the classical [Gaussian linear model]{.blue}, we have that $\theta_i = \mu_i$ and $b(\theta_i) = \theta_i^2/2$. Thus $\theta(y_i) = y_i$ and
$$
\begin{aligned}
...
\end{aligned}
$$
- In the Gaussian case, the deviance is the [residuals sum of squares]{.orange} and $D(\bm{y}; \hat{\bm{\mu}}) = X^2$. 
...
- Let us consider a [Binomial regression]{.blue} model $m_i Y_i = S_i \sim \text{Binomial}(m_i, \pi_i)$ with $\mu_i = \pi_i$. Then
$$
...
- The deviance is useful a [descriptive measure]{.orange} for the [goodness of fit]{.blue}. 
...
- Unfortunately, whenever $\hat{\phi}$ is [estimated]{.orange} we obtain $D(\bm{y}; \hat{\bm{\mu}})/\hat{\phi} \approx n - p$, and exactly $n-p$ in the Gaussian case, so this strategy can not be used.

. . .

- On the other hand, for example in Poisson e binomial regression, we have $\phi = 1$. Hence, the intuition tells us that, at least approximately, we should have
$$
...
:::callout-warning
Then the [deviance]{.blue} $D(\bm{y}; \hat{\bm{\mu}})$ and the $X^2$ [Pearson statistic]{.blue} are approximately distributed as a $\chi^2_{n-p}$ in the following cases:
...
- These indices are [difficult to interpret]{.orange} and could mislead those accustomed with standard $R^2.$ A pseudo-$R^2 \approx 0.4$ may indicate a nearly perfect fit (i.e. `Beetles` data), which is confusing. 

- On top of this, these pseudo-$R^2$ produce different answers depending on the [aggregation]{.blue} of the data. 
...
- Using the $X^2$ statistic with $6$ degrees of freedom, we obtain the p-value $\mathbb{P}(X^2 > 10.027) = 0.124$, as pictured above, which can be interpreted as a [slight]{.orange} lack of fit. 
...
- Using the $X^2$ statistic with $12$ degrees of freedom, we obtain the p-value $\mathbb{P}(X^2 > 29.92) = 0.0028$, therefore [rejecting]{.orange} the hypothesis that this model has a comparable fit with the saturated model.
...
- A few remedies could be:
  ...
  c. Accounting for [overdispersion]{.blue} using quasi-likelihoods, that is, estimating $\phi$ from the data rather than fixing it to $\phi = 1$.
...
- GLMs do not have an additive decomposition, therefore we need define a good [generalization]{.orange} of [residuals]{.orange}. There are at least $2$ alternatives: Pearson and deviance residuals.
...
- Pearson residuals own their name to the fact that the $X^2$ statistic is obtained as
$$
...
- Pearson residuals are an asymptotic approximation of deviance residuals, therefore these two quantities are often very similar in practice. 
...
- Indeed, for example under small dispersion asymptotics or other specific, Pearson residuals are [approximately]{.orange} Gaussian. 
...
- Actually, the analysis for the residuals in [binary data]{.blue} do [not provide useful information]{.orange}; see e.g. @Salvan2020, Section 3.6. 
...
- An outlier might be detected as such simply as the consequence of [model misspecification]{.orange}, e.g. an omitted variable.  
...
- A formal test already confirmed that there are [no]{.orange} noticeable [differences]{.orange} between this model and the saturated model. The analysis of the residuals confirms it.
...
:::callout-tip
Whenever possible, i.e. when $p$ is not too large, we recommend [manually]{.blue} performing each stage of forward or backward procedures and [avoid fully automatic procedures]{.orange}. 
:::
...
- Evaluating many terms increases risk of [chance findings]{.blue}. If true effects are [weak]{.orange}, the largest sample effect likely [overestimates]{.blue} the truth.  

. . .

- Use of standard significance tests in selection lacks [theoretical justification]{.orange}. Distribution of minimum or maximum $p$-values at each stage is not the same as the distribution of a [pre-selected variable]{.blue}. This issue is called [multiple testing]{.blue} and leads to [overconfident]{.orange} conclusions.
