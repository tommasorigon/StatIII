...
## `Beetles` data, from @Bliss1935 

- The `Beetles` dataset originates from @Bliss1935. It records the number of adult flour beetles that died after a 5-hour exposure to gaseous carbon disulphide.
...
- There is a clear [positive]{.orange} and [non-linear]{.blue} relationship between the [proportion of deaths]{.orange} and the logdose. The response variable takes values in $[0, 1]$. 
...
- It is natural to model each $S_i$ as [independent binomial]{.blue} random variables, counting the number of deaths out of $m_i$ individuals. In other words:
$$
S_i \overset{\text{ind}}{\sim} \text{Binomial}(m_i, \pi_i), \qquad i = 1,\dots,8,
$$
where $\pi_i$ is the [probability]{.orange} of death at a given dose $x_i$. Moreover, let $Y_i = S_i / m_i$ be the proportion of deaths, then:
$$
...
## `Beetles` data, fitted model {#beetles-data-fitted-model}
...
## A comparison with old tools I {#beetles-old-tools-i}
...
- The [additive structure]{.blue} $Y_i = \beta_1 + \beta_2 x_i + \epsilon_i$ cannot hold with [iid]{.blue} errors $\epsilon_i$, because $S_i$, and thus $Y_i$, are [discrete]{.orange}. As a result, the errors are always [heteroscedastic]{.orange}. 
...
## A comparison with old tools II {#beetles-old-tools-ii}

:::callout-tip
We consider the [empirical logit]{.blue} variable transformation of $Y_i = S_i / m_i$, obtaining  
$$
...
$$
\hat{\pi}_i = g^{-1}(\hat{\beta}_1 + \hat{\beta}_2 x_i) = \frac{\exp(\hat{\beta}_1 + \hat{\beta}_2 x_i)}{1 + \exp(\hat{\beta}_1 + \hat{\beta}_2 x_i)},$$ 
in which $\hat{\beta}_1$ and $\hat{\beta}_2$ are estimated with OLS using $\text{logit}(\tilde{Y}_i)$ as response. 
:::
...
- Inference is problematic and requires further corrections, because of [heteroscedastic]{.orange} errors. 

- This approach is [not compatible]{.orange} with the reasonable assumption $S_i \sim \text{Binomial}(m_i, \pi_i)$.

## A comparison with old tools III {#beetles-old-tools-iii}
...
- The black line is the predicted curve of a [logistic regression GLM]{.grey}. The orange line is the predicted curve of a [linear model]{.orange}. The blue line is the predictive curve of a [linear model]{.blue} after an [empirical logit variable transformation]{.blue}.

## `Aids` data

- Number of AIDS `deaths` in Australia in a sequence of three-month periods between 1983 and 1986.
...
$$
which increases with $x$, as desired. This implies that $Y_1,\dots,Y_n$ are [heteroscedastic]{.orange}, but this is not an issue in GLMs, as this aspect is [automatically accounted]{.blue} for.


## `Aids` data, fitted model {#aids-data-fitted-model}
...
## A comparison with old tools I {#aids-old-tools-i}
...
$$
\hat{\mu}_i = (\hat{\beta}_1 + \hat{\beta}_2 x_i)^2,$$ 
...
- This approach is [not compatible]{.orange} with the reasonable assumption $Y_i \sim \text{Poisson}(\mu_i)$ and it is only valid as an [asymptotic approximation]{.blue}. 
...
## A comparison with old tools II {#aids-old-tools-ii}
...
- [Random component]{.orange}. This specifies the probability distribution of the response variable $Y_i$. The observations $\bm{y} =(y_1,\dots,y_n)$ on that distribution are treated as [independent]{.orange}. 

. . .

- [Linear predictor]{.blue}. For a parameter vector $\beta = (\beta_1,\dots,\beta_p)^T$ and an $n \times p$ design matrix $\bm{X}$, the linear predictor is $\bm{\eta} = \bm{X}\beta$. We will also write
$$
...
:::callout-tip
In a GLM the observations are independent draws from a distribution $\text{ED}(\mu_i, a_i(\phi)v(\mu_i))$:
$$
...
$$
The unknown variance $\sigma^2 = \phi$ is called [dispersion]{.orange} in GLMs. The [parameter space]{.orange} is $\mathcal{M} = \mathbb{R}$, whereas $a_i(\phi) = \phi$ and the variance function is [constant]{.blue} $v(\mu) = 1$ (homoscedasticity).
:::
...
$$
Y_i \overset{\text{ind}}{\sim}\text{Poisson}(\mu_i), \qquad \log(\mu_i) =  \bm{x}_i^T\beta.
$$
...
- Figure 1 of @Efron2023. [Three levels]{.grey} of statistical modeling. 
...
- By specifying the functions $a_i(\cdot), b(\cdot)$ and $c(\cdot)$ one obtains a particular [parametric model]{.orange}. 
...
- The [score]{.blue} and [Hessian]{.orange} functions, namely the first and second derivatives with respect to $\theta_i$, are
$$
...
- We have $b''(\cdot) > 0$ because $\text{var}(Y_i) > 0$, which means that $b(\cdot)$ is a [convex function]{.blue}. 

- Moreover, the function $b'(\theta)$ is [continuous]{.orange} and [monotone increasing]{.orange} and hence [invertible]{.orange}. 

:::aside
The function $b(\cdot)$ is related to the moment generating function of $Y_i$. Thus, higher order derivatives of $b(\cdot)$ allow the calculation of skewness, kurtosis, etc. 
:::
...
Let $Y_i \sim \text{Poisson}(\mu_i)$. The [pmf]{.blue} of $Y_i$ can be written as
$$
\begin{aligned}
...
&=\exp\left\{\frac{\theta_i y_i + \log(-\theta_i)}{\phi} - (1/\phi)\log{\phi}+ (1/\phi - 1)\log{y_i}  - \log{\Gamma}(1/\phi)\right\}, \qquad y_i > 0,\\
\end{aligned}
$$
...
\end{aligned}
$$
for $y_i \in \{0, 1/m_i, 2/m_i, \dots, 1\}$. This can be written as
$$
p(y_i; m_i, \pi_i) =\exp\left\{\frac{y_i\theta_i - \log\{1 + \exp(\theta_i)\}}{1/m_i}+ \log\binom{m_i}{m_i y_i}\right\},
$$
where the [natural parameter]{.orange} is $\theta_i = \text{logit}(\pi_i) = \log\{\pi_i/(1-\pi_i)\}$. 
...
- Note that $\mathbb{E}(Y_i) = \mathbb{E}(S_i / m_i) = \pi_i = \mu_i$. This means there is [no dispersion]{.orange} parameter $\phi$ and
$$
...
\end{aligned}
$$
from which we obtain that the [variance function]{.blue} $v(\mu_i) = \mu_i(1-\mu_i)$ is quadratic. 
...
|--------------------|---------------|---------------|---------------|---------------|
| [Support]{.blue} $\mathcal{Y}$ |  $\mathbb{R}$| $(0, \infty)$ | $\{0, 1/m_i,\dots, 1\}$ | $\{0, 1, 2, \dots\}$ |
| $\theta_i = \theta(\mu_i)$ | $\mu_i$ | $- 1/\mu_i$ | $\log\left(\frac{\mu_i}{1 - \mu_i}\right)$ | $\log{\mu_i}$ |
| [Parameter space]{.orange} $\Theta$ | $\mathbb{R}$ | $(-\infty, 0)$ | $\mathbb{R}$ | $\mathbb{R}$ | 
...
- For example, in [binary regression]{.blue} any continuous [cumulative distribution function]{.orange} for $g^{-1}(\cdot)$ leads to a good link function, such as $g^{-1}(\cdot) = \Phi(\cdot)$ (probit) or $g^{-1}(\eta_i) = e^{\eta_i}/(1 + e^{\eta_i})$ (logistic). 
...
- Let $Y_i \overset{\text{ind}}{\sim}\text{ED}(\mu_i, a_i(\phi)v(\mu_i))$ be the response variable of a GLM, with $g(\mu_i) = \bm{x}_i^T\beta$. The [joint distribution]{.blue} of the responses $\bm{Y} = (Y_1,\dots,Y_n)^T$ is
$$
...
$$
\ell(\beta, \phi) = \sum_{i=1}^n\left\{\frac{y_i\theta_i - b(\theta_i)}{a_i(\phi)} + c(y_i, \phi)\right\}.
$$
...
$$
Moreover, since $\mu_i = g^{-1}(\eta_i)$ we obtain 
$$
\frac{\partial \mu_i}{\partial \eta_i} = \frac{1}{g'(g^{-1}(\eta_i))} = \frac{1}{g'(\mu_i)}.
$$
- Summing up, the [chain rule of differentiation]{.orange} for [composite functions]{.blue} gives:
$$
...
- Combining all the former equations, we obtain an explicit formula for the [score function]{.blue}
$$
...
## Canonical link: simplifications 📖 {#canonical-link-score}
...
$$
Thus, plugging this equality into the previous equations gives:
$$
...
:::callout-tip
Let $Y_i \sim \text{ED}(\mu_i, \phi/\omega_i)$ with $g(\mu_i) = \mu_i$, namely the [Gaussian]{.orange} linear model with the [identity]{.grey} (canonical) link and [heteroscedastic errors]{.blue}. The likelihood equations are
$$
\bm{X}^T\bm{\Omega}(\bm{y} - \bm{X}\beta) = \bm{0},
$$
Their solution over $\beta$ is the weighted least squares estimator $\hat{\beta}_\text{wls} = (\bm{X}^T\bm{\Omega}\bm{X})^{-1}\bm{X}^T\bm{\Omega}\bm{y}$. 
:::
...
$$
\bm{X}^T(\bm{y} - \bm{\mu}) = \bm{0}, \qquad \bm{\mu} = (e^{\bm{x}_1^T\beta}, \dots,  e^{\bm{x}_n^T\beta})^T. 
$$
:::

## Example: `Beetles` data {#beetles-likelihood-equations}
...
$$
\hat{\beta} = (\hat{\beta}_1, \hat{\beta}_2) = (-60.717, 34.270).
$$



## Example: `Beetles` data {#beetles-predictions}
...
- The predicted values and the data $Y_i$ were also shown in a [plot]{.blue} at the [beginning of this unit](#beetles-data-fitted-model).


## Example: `Aids` data {#aids-likelihood-equations}

- In the `Aids` data, we specified a [Poisson]{.blue} regression model with $\mathbb{E}(Y_i) = \exp(\beta_1 + \beta_2 x_i)$. 

- The maximum likelihood estimate $(\hat{\beta}_1, \hat{\beta}_2)$ [solves]{.orange} simultaneously:
$$
\sum_{i=1}^n y_i =  \sum_{i=1}^n \exp(\beta_1 + \beta_2x_i), \quad
\text{and}\quad \sum_{i=1}^n x_i y_i =  \sum_{i=1}^n x_i\exp(\beta_1 + \beta_2 x_i).
$$
- This system does [not]{.orange} always admit a [solution]{.orange}. This happens, for example, in the [extreme case]{.blue} $\sum_{i=1}^ny_i = 0$, occurring when all counts equal zero.
...
$$
In other words, the estimated number of deaths increases by about $30\%$ every quarter.
...
## Example: `Aids` data {#aids-predictions}
...
- The predicted values and the data $Y_i$ were also shown in a [plot]{.blue} at the [beginning of this unit](#aids-data-fitted-model).
...
\frac{\partial\mu_i}{\partial \beta_s} = \frac{x_{is}}{g'(\mu_i)}, \qquad \frac{\partial\theta_i}{\partial \beta_r} = \frac{x_{ir}}{v(\mu_i) g'(\mu_i)}.
$$
...
## Canonical link: simplifications 📖 {#canonical-link-information}

:::callout-note
Under the [canonical link]{.orange} we have that $\theta_i = x_{i1}\beta_1 + \cdots +\beta_p x_{ip}$, which means that
$$
...
as in @Agresti2015. Of course, the final result [coincides]{.orange} with ours. 



## ☠️ - Orthogonality of $\beta$ and $\phi$
...
:::callout-note
The parameters $\beta$ and $\phi$ are [orthogonal]{.orange} and their estimates are [asymptotically independent]{.blue}.

Moreover, the matrices $\bm{I}$ and $\bm{I}^{-1}$ are all that is needed for inference on $\beta$, so there is no need to compute $i_{\phi \phi}$. Moreover, the maximum likelihood $\hat{\beta}$ can also be computed without knowing $\phi$. 
:::
...
- The algorithm [stops]{.orange} whenever a certain criterion is met, e.g. when $||\beta^{(t+1)} - \beta^{(t)}|| < \epsilon$, where $\epsilon$ is sometimes called [tolerance]{.grey}. We say it reached [convergence]{.blue}. 
...
$$
\beta^{(t+1)} = \beta^{(t)} + (\bm{J}^{(t)})^{-1}\ell_*(\beta^{(t)}), \qquad t=1,2,\dots
$$

:::callout-tip
The Newton-Raphson method essentially considers a series of [parabolic approximations]{.orange} to the log-likelihood, each time evaluating the [point of maximum]{.blue}.
:::
...
- The above formula can be [simplified]{.blue} a bit. First, we rewrite the [score]{.orange} as
$$
...
- Exploiting the above formulas, we can write the IRLS update as follows
$$
...
$$
where $\bm{z}^{(t)} = (z_1^{(t)}, \dots, z_n^{(t)})^T$ is called [pseudo-response]{.orange} whose elements are defined as
$$
z_i^{(t)} = \bm{x}_i^T\beta^{(t)} + u_i^{(t)}  =  \bm{x}_i^T\beta^{(t)} + (y_i - \mu_i^{(t)})g'(\mu_i^{(t)}),\qquad i=1,\dots,n.
$$
Hence, each update can be interpreted as the solution of a [weighted least squares]{.grey} problem:
$$
...
- The IRLS update does not depend on the choice of $\phi$, because it [cancels]{.orange} in the multiplications, as we would expect. 
...
- Based on this approximation, a good [initialization]{.blue} is obtained by setting $\bm{W} = I_n$ and $z_i = g(y_i)$, which gives
$$
\beta^{(1)} = (\bm{X}^T\bm{X})^{-1}\bm{X}^Tg(\bm{y}),
$$
namely the [least squares]{.orange} solution for the transformed data. To avoid boundary issues, sometimes the data are perturbed, as we did in binomial regression. 
...
-  [Initialize]{.grey} $\beta^{(1)} = (\bm{X}^T\bm{X})^{-1}\bm{X}^T\text{logit}(\tilde{\bm{y}})$ where $\text{logit}(\tilde{\bm{y}})$ is the [empirical logit]{.orange} transform.
...
- Because of [numerical instabilities]{.orange} and [lack of robustness]{.blue}, a [method of moments]{.orange} estimator is typically preferred to the maximum likelihood one. If $\mu_i$ were known, the estimator
$$
...
<!-- - Note that the asymptotic variance implicitly takes into account [heteroscedasticity]{.orange}.  -->

## Example: `Beetles` data {#beetles-standard-errors}
...
## Example: `Aids` data {#aids-standard-errors}
...
- By inverting the Wald test, we obtain the associated [confidence interval]{.blue}
$$
...
## Comparison with the Gaussian linear model {#gaussian-comparison-wald}

- In a classical [Gaussian linear model]{.blue} the weight matrix is $\bm{W} = I_n/\sigma^2$, therefore
$$
...
- However, in the Gaussian case there is [no need]{.orange} of [approximations]{.orange}. The distribution of $z_j$ is a [Student]{.grey}'s $t_{n-p}$ under $H_0$, which indeed converges to a $\text{N}(0, 1)$ for large values of $n$.
...
## Example: `Beetles` data {#beetles-wald-test}
...
## Example: `Aids` data {#aids-wald-test}
...
Let $\hat{\beta} = (\hat{\beta}_A, \hat{\beta}_B)$ be the [unrestricted]{.blue} maximum likelihood estimate; the quantity
$$
W_e = (\hat{\beta}_B - \beta_0)^T \:\widehat{\text{var}}(\hat{\beta}_B)^{-1} \:(\hat{\beta}_B - \beta_0),
$$
is called [Wald test]{.blue}. Here $\widehat{\text{var}}(\hat{\beta}_B)$ is the appropriate [block]{.grey} of $(\bm{X}^T\hat{\bm{W}}\bm{X})^{-1}$ and $\hat{\bm{W}}$ is estimated using $\hat{\beta}$ and $\hat{\phi}$. Under $H_0$ this quantity is [approximately]{.orange} distributed as
$$
W_e \: \dot{\sim} \; \chi^2_q,
$$
...
Let $\hat{\beta} = (\hat{\beta}_A, \hat{\beta}_B)$ be the [unrestricted]{.blue} maximum likelihood estimate and let $\hat{\beta}_0 = (\hat{\beta}_{A,0}, \textcolor{red}{\beta_0})$ be the [restricted]{.orange} maximum likelihood estimate. The quantity
$$
...
:::callout-note
#### Rao-score test
Let $\hat{\beta} = (\hat{\beta}_A, \hat{\beta}_B)$ be the [unrestricted]{.blue} maximum likelihood estimate and let $\hat{\beta}_0 = (\hat{\beta}_{A,0}, \textcolor{red}{\beta_0})$ be the [restricted]{.orange} maximum likelihood estimate. Moreover, let
$$
...
$$
is called [Rao-score test]{.blue}. Here $\widetilde{\text{var}}(\hat{\beta}_B)$ is the appropriate [block]{.grey} of $(\bm{X}^T\tilde{\bm{W}}\bm{X})^{-1}$ where $\tilde{\bm{W}}$ is estimated using the [restricted]{.orange} $\hat{\beta}_0$. Under $H_0$ this quantity is [approximately]{.orange} distributed as
$$
W_u \: \dot{\sim} \; \chi^2_q,
$$
...
:::aside
The Rao-score test is arguably the [least common]{.orange}. When $\phi$ is unknown, there are several variants depending on how it is estimated. 
:::
...
- Figure taken from @Azzalini2008. This is also the [cover]{.grey} of the [book]{.orange}!


## Three asymptotically equivalent tests

:::callout-warning
The Wald test, the Score test and the log-likelihood ratio test are [asymptotically equivalent]{.blue}, that is, these tests give the [same number]{.blue} for large values of $n$. We have that
$$
...
[^wald_delta]: Transforming the extremes of the Wald confidence interval "works" in the sense that it produces a valid confidence interval, but it is [not]{.orange} the Wald interval in the transformed scale. 

## Comparison with the Gaussian linear model {#gaussian-comparison-tests}
...
$$
The $\chi^2_q$ distribution is [exact]{.orange} and not an approximation thanks to Cochran's theorem. 
:::
...
$$
\begin{aligned}
W &= 2 [\ell(\hat{\beta}, \hat{\phi}) - \ell(\hat{\beta}_0, \hat{\phi})] = \frac{||\bm{Y} - \bm{X}\hat{\beta}_0||^2 - ||\bm{Y} - \bm{X}\hat{\beta}||^2}{\hat{\phi}} = q \frac{(||\bm{Y} - \bm{X}\hat{\beta}_0||^2 - ||\bm{Y} - \bm{X}\hat{\beta}||^2)/q}{||\bm{Y} - \bm{X}\hat{\beta}||^2/(n - p)} \\
& = q F,
\end{aligned}
$$
...
## Example: `Beetles` data {#beetles-three-tests}
...
## Example: `Beetles` data {#beetles-confidence-intervals}
...
- The Wald interval was also computed before. The three tests produce [nearly identical]{.blue} intervals.  
...
## Example: `Aids` data {#aids-three-tests}
...
```
 ...
 - Let us now perform the same analysis for the `Aids` data. Again, we test the null hypothesis $H_0 : \beta_2 = 0$, which is the relevance of `period` in predicting the response.
...
## Example: `Aids` data {#aids-transformed-intervals}
...
- Thanks to the [invariance property]{.blue} of the Rao-score and the log-likelihood ratio tests, we can simply [transform]{.orange} the [original intervals]{.blue} for $\beta_2$.

- If the extremes of the log-likelihood ratio interval are $C_\text{low}, C_\text{high}$, then the new interval is
$$
...
- The [average]{.orange} percentage increase is between $24\%$ and $35\%$ each `period`, with a $95\%$ [confidence]{.blue}. 

- These confidence intervals are always [positive]{.orange}, which is desirable because they are percentages. 

## Example: `Aids` data {#aids-delta-method}

- In the Wald case, we cannot simply transform the extremes of the intervals. Indeed, that would lead to a [valid]{.blue} confidence interval that is no longer of Wald type (*Lo sbagliato* 🥃).
...
- The [empirical proportions]{.orange} $s_i / m_i$ can be seen as [estimates]{.blue} of the [most flexible model]{.blue}, in which every observation $Y_i$ has its own mean $\mu_i$. We call it the [saturated model]{.orange} because $p = n$.

## Saturated model

- Let us express the [log-likelihood]{.orange} of a GLM as a function of the mean $\bm{\mu} = (\mu_1,\dots,\mu_n)^T$.

- When evaluated in the maximum likelihood, this gives:
$$
\ell_\mathcal{M}(\hat{\bm{\mu}},\phi) = \sum_{i=1}^n\left\{\omega_i\frac{y_i\theta(\hat{\mu}_i) - b(\theta(\hat{\mu}_i))}{\phi} + c(y_i, \phi)\right\}.
$$
The maximum likelihood for each $\mu_i$ is [restricted]{.orange}, in the sense that it depends on the $p$ parameters of the [linear predictor]{.blue} $\bm{x}_i^T\beta$ through the link function $g(\mu_i) = \bm{x}_i^T\beta$. 
...
$$
\ell_\mathcal{M}(\bm{y},\phi) = \sum_{i=1}^n\left\{\omega_i\frac{y_i\theta(y_i) - b(\theta(y_i))}{\phi} + c(y_i, \phi)\right\}.
$$
...
- There is also a strong parallelism with the Gaussian linear model.
...
:::{.callout-note}
Let us consider a model $M_\text{null}$ with [no covariates]{.blue} and one parameter ($p =1$), i.e. the [intercept]{.blue}. The predicted values are all equal to
$$
\hat{\bm{\mu}}_\text{null} = (g^{-1}(\hat{\beta}_1), \dots, g^{-1}(\hat{\beta}_1))^T.
$$
...
$$
If $H_0$ is not rejected, it means that all the [covariates]{.orange} are regarded as [irrelevant]{.orange}. 

## Pearson $X^2$ statistic

- The deviance is a [log-likelihood ratio]{.blue} test between a given model and the saturated model, rescaled by $\phi$.

- Hence, we may consider another test, like the [Rao-Score]{.orange}, to obtain an alternative definition.
...
- In the classical [Gaussian linear model]{.blue}, we have that $\theta_i = \mu_i$, $b(\theta_i) = \theta_i^2/2$ and $\omega_i = 1$. Thus $\theta(y_i) = y_i$ and
$$
\begin{aligned}
...
&= \sum_{i=1}^n(y_i^2 - 2y_i\hat{\mu}_i + \hat{\mu}_i^2)= \sum_{i=1}^n(y_i - \hat{\mu}_i)^2.
\end{aligned}
$$
- In the Gaussian case, the deviance is the [residual sum of squares]{.orange} and $D(\bm{y}; \hat{\bm{\mu}}) = X^2$. 
...
\end{aligned}
$$
with the [convention]{.grey} that $y_i\log(y_i/\hat{\mu}_i) = 0$ whenever $y_i = 0$. 
...
$$
As discussed in @Salvan2020, Example 2.12, this can be seen as a [quadratic approximation]{.orange} of the [deviance]{.blue}, which is valid for large values of $n$. 

## Deviance of a binomial model I

- Let us consider a [Binomial regression]{.blue} model $m_i Y_i = S_i \sim \text{Binomial}(m_i, \pi_i)$ with $\mu_i = \pi_i$. Then, up to an [additive constant]{.grey} not depending on $\bm{\mu}$,
$$
\ell_\mathcal{M}(\bm{\hat{\mu}}) = \sum_{i=1}^n\{m_i y_i \log{(\hat{\mu}_i)} + m_i(1 - y_i)\log{(1-\hat{\mu}_i)}\}.
$$
Therefore, under the convention $x\log(x) = 0$ as before, the deviance is
$$
\begin{aligned}
...
- The quantities $m_i y_i$ and $m_i - m_i y_i$ can be interpreted as the number of observed [successes]{.blue} and [failures]{.orange}, respectively. Similarly, $m_i\hat{\mu}_i$ and $m_i - m_i\hat{\mu}_i$ represent their [predictions]{.grey}. Hence, we can write
$$
...
- The $X^2$ statistic of a binomial model, recalling that $v(\mu_i) = \mu_i(1 - \mu_i)$, equals to
$$
\begin{aligned}
...
\end{aligned}
$$
The second representation follows after some [algebra]{.grey}. 

- The second equation shows that we can write
$$
X^2 = \sum_{j=1}^{2n} \frac{(\textsf{observed}_j - \textsf{fitted}_j)^2}{\textsf{fitted}_j}.
$$

- As already mentioned, the $X^2$ statistic can be seen as a [quadratic approximation]{.orange} of the [deviance]{.blue}. 

## Deviance as goodness of fit measure I

- The deviance is a useful [descriptive measure]{.orange} of the [goodness of fit]{.blue}. 

- It is [tempting]{.orange} to use the deviance as a formal [statistical test]{.blue}, to verify if the current model is adequate compared to the saturated model. 

 . . .
 
- Suppose $\phi$ were [known]{.blue}, then in the Gaussian case we would have
$D(\bm{y}; \hat{\bm{\mu}})/\phi \sim \chi^2_{n-p}$, which would allow us to check the adequacy of the model. 

- Unfortunately, whenever $\hat{\phi}$ is [estimated]{.orange} we obtain $D(\bm{y}; \hat{\bm{\mu}})/\hat{\phi} \approx n - p$, and exactly $n-p$ in the Gaussian case, so this strategy cannot be used.

. . .

- On the other hand, for example in Poisson and binomial regression, we have $\phi = 1$. Hence, intuition tells us that, at least approximately, we should have
$$
D(\bm{y}; \hat{\bm{\mu}}) \; \dot \sim \; \chi^2_{n-p}.
$$

- Unfortunately, this is [not]{.orange} the [case]{.orange}: the saturated model is an "irregular case" in that the number of parameters $p = n$ grows with the sample size. 
...
- Despite this bad news, it turns out that in some special cases, the $\chi^2_{n-p}$ approximation is still valid even for fixed values of $n$.

:::callout-warning
The [deviance]{.blue} $D(\bm{y}; \hat{\bm{\mu}})$ and the $X^2$ [Pearson statistic]{.blue} are approximately distributed as a $\chi^2_{n-p}$ in the following cases:

  - In binomial regression, for large values of $m_i$ (small-dispersion asymptotics);
  - In Poisson regression, for large values of the estimated means $\hat{\mu}_i$ (say $\hat{\mu}_i \ge 5$);
  - When $\phi$ is known and $\phi \rightarrow 0$ (small-dispersion asymptotics).
:::

- [Small-dispersion asymptotics]{.blue} describe an alternative limiting regime in which the variance of the observations tends to $0$.

- In binomial regression the $X^2$ statistic converges to $\chi^2_{n-p}$ more quickly than the deviance and has a [more trustworthy]{.blue} p-value when some expected success or failure totals are less than about five.

- The $\chi^2_{n-p}$ approximation is [very poor]{.orange} for binary regression, i.e. when $m_i = 1$.

## On pseudo-$R^2$

- There exist several generalizations of the $R^2$ statistic for linear models, called [pseudo-$R^2$]{.blue} (e.g. McFadden, Cox \& Snell, Nagelkerke, Tjur, etc.).

- These indices are [difficult to interpret]{.orange} and could mislead those accustomed to the standard $R^2$. A pseudo-$R^2 \approx 0.4$ may indicate a nearly perfect fit (e.g. the `Beetles` data), which is confusing. 

- On top of this, these pseudo-$R^2$ measures produce different answers depending on the [aggregation]{.blue} of the data. 

. . .

- The recommendation is to rely on indices tailored to the data at hand, such as the ROC curve for binary data, or the correlation between $\bm{y}$ and $\hat{\bm{\mu}}$.

- The residual [deviance]{.blue} is also a useful tool, especially for comparing models.
...
- Pseudo-$R^2$ measures are often shown by default in other software, such as SAS or SPSS. 

## Example: `Beetles` data, output of `summary`

- This is what the `summary` of a GLM looks like. It is very similar to the `summary` of `lm`. At this stage of the course, you should be able to understand almost everything.
...
- `anova(model0, model1)` computes the [log-likelihood ratio test]{.blue} comparing two nested models: the [reduced]{.orange} model $M_0$ with $p_0$ parameters and the [full]{.blue} model $M_1$ with $p$ parameters.
...
- `Deviance` indicates the [change in deviance]{.blue}, that is $\phi W = D(\bm{y}; \hat{\bm{\mu}}_0) - D(\bm{y}; \hat{\bm{\mu}})$.
...
- The [deviance]{.blue} equals $D(\bm{y}; \hat{\bm{\mu}}) = 11.232$, with $n - p = 8 - 2 = 6$ degrees of freedom. The observed $X^2$ [Pearson statistic]{.orange} equals $10.027$. 

- Using the $X^2$ statistic with $6$ degrees of freedom, we obtain the p-value $\mathbb{P}(\chi^2_6 > 10.027) = 0.124$, as pictured above. There is therefore [no evidence]{.blue} of lack of fit: this model is [compatible]{.blue} with the saturated one. 


## Example: `Aids` data, output of `summary`

- The `summary` of the Poisson regression model for the `Aids` data is shown below.
...
:::callout-tip
The use of the term `deviance` to indicate the [difference]{.blue} between two deviances is a bit [misleading]{.orange}. I do not know the reason, but the `lrtest` function of the `lmtest` package changed it.
:::
...
## Example: `Aids` data, goodness of fit

```{r}
X_Aids <- sum(residuals(fit_Aids, type = "pearson")^2)
```

- The [deviance]{.blue} equals $D(\bm{y}; \hat{\bm{\mu}}) = 30.203$, with $n - p = 14 - 2 = 12$ degrees of freedom. The observed $X^2$ [Pearson statistic]{.orange} equals $29.92$.

- Using the $X^2$ statistic with $12$ degrees of freedom, we obtain the p-value $\mathbb{P}(\chi^2_{12} > 29.92) = 0.0028$, therefore [rejecting]{.orange} the hypothesis that this model has a fit comparable to the saturated one.

. . .

- The $X^2$ goodness of fit test indicates a potential issue with the model, but it does not explain [why]{.blue}. 

- A few remedies could be:
  
  a. Choosing a different [link function]{.orange};
  b. Including an [additional covariate]{.blue} (if available), and/or considering non-linear [transformations]{.orange} of the available ones;
  c. Choosing a different [distribution]{.orange} instead of the Poisson, such as the negative binomial;
  d. Accounting for [overdispersion]{.blue} using quasi-likelihoods, that is, estimating $\phi$ from the data rather than fixing it to $\phi = 1$.
...
- Linear models have an additive structure $y_i = \bm{x}_i^T\beta + \epsilon_i$ therefore the residuals can be estimated as
$$
r_i = y_i - \hat{\mu}_i.
$$
We call these the [response residuals]{.blue}.

. . .

- GLMs do not have an additive decomposition, therefore we need to define a good [generalization]{.orange} of [residuals]{.orange}. There are at least two alternatives: Pearson and deviance residuals.
...
- The analysis of the residuals is very helpful for identifying any misspecification, as well as for hinting at the solution. In particular, it is useful for instance for:
  a. Choosing the correct variance function $v(\mu_i)$, i.e. the correct response distribution;
  b. Choosing the correct link function;
  c. Identifying latent patterns, often an indication of an omitted variable;
  d. Identifying potential outliers and leverage points.
...
:::callout-note
For a GLM with variance function $v(\mu_i)$, we call [Pearson residuals]{.blue} the following quantities:
$$
r_{i, P} = \frac{y_i - \hat{\mu}_i}{\sqrt{v(\hat{\mu}_i)/\omega_i}}, \qquad i=1,\dots,n.
$$
:::

- Pearson residuals rescale the response residuals, accounting for [heteroscedasticity]{.orange}. 

- Pearson residuals owe their name to the fact that the $X^2$ statistic is obtained as
$$
X^2 = \sum_{i=1}^n r_{i, P}^2 = \sum_{i=1}^n \omega_i \frac{(y_i - \hat{\mu}_i)^2}{v(\hat{\mu}_i)}.
$$
Moreover, the dispersion parameter, when present, can be estimated as
$$
...
The deviance of a GLM can be written as a sum of individual contributions $D(\bm{y}; \hat{\bm{\mu}}) = \sum_{i=1}^n d_i$ and 
$$
d_i = 2 \omega_i\left\{y_i [\theta(y_i) - \theta(\hat{\mu}_i)] - [b(\theta(y_i)) - b(\theta(\hat{\mu}_i))]\right\}, \qquad i=1,\dots,n.
$$
We call [deviance residuals]{.blue} the following quantities:
$$
r_{i, D} = \text{sign}(y_i - \hat{\mu}_i) \sqrt{d_i}, \qquad i=1,\dots,n.
$$
:::

- By definition, the deviance is obtained as
$$
D(\bm{y}; \hat{\bm{\mu}}) = \sum_{i=1}^n r_{i, D}^2. 
$$
- Deviance residuals are the [default]{.orange} choice in the `residuals` **R** function. 

- Pearson and deviance residuals are [asymptotically equivalent]{.blue}, therefore these two quantities are often very similar in practice. 


## A weighted projection matrix

- In linear models, we considered the [hat matrix]{.blue} $\bm{H} = \bm{X}(\bm{X}^T\bm{X})^{-1}\bm{X}^T$, whose diagonal elements $h_i$ are called [leverages]{.orange} and it holds $\text{var}(r_i) = \sigma^2(1 - h_i)$. 

. . .

- Several arguments can be invoked to justify the following [weighted hat matrix]{.blue} in GLMs
$$
\bm{H}_W = \bm{W}^{1/2}\bm{X}(\bm{X}^T\bm{W}\bm{X})^{-1}\bm{X}^T\bm{W}^{1/2}.
$$
This matrix is [symmetric]{.blue} $(\bm{H}_W = \bm{H}_W^T)$ and [idempotent]{.orange} ($\bm{H}_W^2 = \bm{H}_W$), i.e. a projection matrix. 

- We denote with $h_{i,W}$ the diagonal elements of $\bm{H}_W$, which are the [leverages]{.blue} of a GLM. In practice $\bm{W}$ is estimated from the data, therefore the leverages will depend on the [response]{.orange}. 

- It can be shown, as in @Agresti2015, Section 4.4.5, that 
$$
...
- A technical but deeper discussion about $\bm{H}_W$ can be found in the Appendix of Chapter 4 of @Agresti2015, where stronger analogies with linear models are shown. 

## Standardized residuals

- In analogy with what has been done for linear models, we can consider the [standardized]{.blue} version of Pearson and deviance residuals.

- [Standardized Pearson residuals]{.blue} are defined as
$$
\tilde{r}_{i, P} = \frac{r_{i,P}}{\sqrt{\hat{\phi}(1 - \hat{h}_{i, W})}} = \frac{y_i - \hat{\mu}_i}{\sqrt{(\hat{\phi}/\omega_i)v(\hat{\mu}_i)(1 - \hat{h}_{i, W})}}, \qquad i=1,\dots,n,
$$
where $\hat{\phi}$ is an estimate of $\phi$ (if unknown) and $\hat{h}_{i, W}$ is an estimate of the leverages. 

- [Standardized deviance residuals]{.blue} are defined as
$$
\tilde{r}_{i, D} = \frac{r_{i, D}}{\sqrt{\hat{\phi} (1 - \hat{h}_{i, W})}}, \qquad i=1,\dots,n.
$$

- We can also obtain an [approximate Cook's distance]{.orange} by considering
$$
...
- Indeed, for example under small dispersion asymptotics or other specific conditions, Pearson residuals are [approximately]{.orange} Gaussian. 

. . .

- However, these conditions are often not met. For example, in binary data the response $y_i \in \{0, 1\}$ can only assume two values and the residuals will not be normally distributed, even for large $n$. 

- Actually, the analysis of the residuals in [binary data]{.blue} does [not provide useful information]{.orange}; see e.g. @Salvan2020, Section 3.6. 

. . .

:::callout-tip
The analysis of residuals in GLMs is often useful but should be taken *cum grano salis*, particularly when dealing with discrete responses that take only a few distinct values.

Overreliance on residual analysis can encourage automatic decisions rather than thoughtful, critical judgment. A good model should not be discarded solely on the basis of a "bad" diagnostic plot.
:::

## On identifying and removing outliers

- The analysis of the residuals can also help in identifying outliers and influential points. However, we must be careful in drawing conclusions.

- An outlier might be detected as such simply as a consequence of [model misspecification]{.orange}, e.g. an omitted variable.  

- In the vast majority of cases, the presence of outliers should be dealt with by carefully modifying the model.

- There are instances in which outliers are actually [contaminated data points]{.blue} (e.g. `age = -3`). If there is strong and contextual evidence that this might be the case, then these points should be [removed]{.orange}. Otherwise, removing data points is a [bad practice]{.orange}.

:::callout-tip
In statistical modelling, we wish to find a model that fits our data, not a dataset that is aligned with our prescribed model. 

Discarding observations until the hypotheses are reasonable skews the overall analysis and does not answer any meaningful scientific or business question. 
:::

## Example: `Beetles` data {#beetles-residuals}
...
plot(predict(fit_Beetles, type = "response"), rstandard(fit_Beetles, type = "deviance"), 
     pch = 16, xlab = expression(mu), ylab = "Standardized deviance residuals"); abline(h = 0, lty = "dotted")
plot(predict(fit_Beetles, type = "response"), rstandard(fit_Beetles, type = "pearson"), 
     pch = 16, xlab = expression(mu), ylab = "Standardized Pearson residuals"); abline(h = 0, lty = "dotted")
par(mfrow = c(1, 1))
```

- The goodness of fit test did not detect any [noticeable difference]{.orange} between this model and the saturated one. The analysis of the residuals confirms it.

- Deviance residuals and Pearson residuals are very similar, as expected. 

## Example: `Beetles` data {#beetles-cook-distance}
...
- The Cook's distance also confirms that there are no strong influential points. 

## Example: `Aids` data {#aids-residuals}
...
- From both residual plots it is evident that three observations are severely underestimated, while the others are slightly overestimated. 

- The lack of fit can be remedied, in this case, by using a different link function.

## Example: `Aids` data {#aids-residuals-sqrt-link}
...
- We estimated a Poisson regression model with a [non-canonical link]{.blue} function $g(\mu_i) = \sqrt{\mu_i}$.

- This yields a much [better fit]{.blue}, as we previously discussed. The residuals are also better behaved. 
...
- [Model selection]{.blue} for GLMs faces the same issues as for linear models. 
...
- The selection process becomes more difficult as the number of explanatory variables $p$ increases, because of the growth in possible effects and interactions. There are two competing goals:  
  ...
  a. The model should be complex enough to [fit the data well]{.blue};
  b. On the other hand, it should [smooth rather than overfit]{.orange} the data and ideally remain [simple to interpret]{.blue}.

. . .

- Most research studies are designed to answer certain questions, which guide the choice. 

- [Confirmatory analyses]{.blue} use a restricted set of models, e.g. for testing a study hypothesis about an effect by comparing models [with and without]{.orange} that effect.  

- [Exploratory studies]{.blue}, instead, search among possible models which may provide clues about the structure of effects or can raise questions for future research.  

. . .

- In either case, it is helpful first to study the [marginal effect]{.blue} of each predictor. Use [descriptive statistics]{.orange} and a scatterplot matrix to get a feel for those effects.  


## Automatic model selection

- With $p$ [explanatory variables]{.blue}, the number of potential models is the huge number
$$
\sum_{k=0}^p \binom{p}{k} = 2^p.
$$
- [Best subset selection]{.blue} identifies the model that optimizes an [information criterion]{.blue} e.g. AIC or BIC, which are defined as
$$
\textsf{AIC} = - 2\ell(\hat{\beta}) + 2k, \qquad\textsf{BIC} = - 2\ell(\hat{\beta}) + k\log{n},
$$
where $k$ is the number of parameters in the model.

- Best subset selection is [computationally intensive]{.orange} when $p$ is large, to the extent that it is not even feasible in most cases, but approximations such as [forward]{.blue} and [backward]{.blue} selection are possible.

. . .

- In [exploratory studies]{.blue}, these methods are useful if applied [cautiously]{.blue}. 

- As we shall discuss, an excess of automatism may lead to good predictive performance, but it may fail to make the model simple or interpretable. 


## Stepwise procedures: forward and backward selection

- [Forward selection]{.blue} adds terms sequentially. At each stage it selects the term giving the greatest improvement in terms of [deviance]{.orange} or other goodness of fit measures. 

- The process stops when further additions do not improve the fit, according to statistical significance (i.e. a log-likelihood ratio test) or a criterion for judging the model fit (such as the AIC or BIC).

- A stepwise variation of this procedure rechecks, at each stage, whether terms added at previous stages are still needed.

. . .

- [Backward elimination]{.orange} begins with a complex model and sequentially removes terms. 
...
- The process stops when any further deletion leads to a poorer fit.

:::callout-tip
Whenever possible, i.e. when $p$ is not too large, we recommend [manually]{.blue} performing each stage of forward or backward procedures and [avoiding fully automatic procedures]{.orange}. 
:::


## Comments on forward/backward selection I

- An [interaction term]{.blue} should not be included without its [main effects]{.orange}.  

- For [qualitative predictors]{.blue} with $>2$ categories: add/drop the [entire variable]{.orange}, not just one indicator. Otherwise, results depend on the reference category used in coding.  

. . .

- Some statisticians prefer [backward elimination]{.blue} over [forward selection]{.orange}. It is safer to delete terms from an overly complex model than to [add]{.orange} to an overly simple one. 

- Forward selection based on significance tests:  
  a. May stop prematurely if a test has [low power]{.blue}.  
  b. Early-stage comparisons often involve [inadequate models]{.orange}, making tests [questionable]{.blue}.  

. . .

- Neither backward nor forward strategies guarantee a [meaningful model]{.blue}.  

- Evaluating many terms increases the risk of [chance findings]{.blue}. If true effects are [weak]{.orange}, the largest sample effect likely [overestimates]{.blue} the truth.  

. . .

- Use of standard significance tests in selection lacks [theoretical justification]{.orange}. The distribution of the minimum or maximum $p$-value at each stage is not the same as the distribution of a [pre-selected variable]{.blue}. This issue is called [multiple testing]{.blue} and leads to [overconfident]{.orange} conclusions.


## Comments on forward/backward selection II


- Statistical significance is not the same as [practical significance]{.blue}; do not rely only on significance tests.  

- The price to pay for adding an irrelevant variable is an increase in [variance]{.blue} of the estimates. The price to pay for dropping a relevant variable is an increase in [bias]{.orange}.

. . .

- It is possible to include variables central to the study goals even if [not significant]{.orange}:  
  - It enables comparisons with other studies where the effect is significant, perhaps because of a larger sample size;
  - If the variable is a potential [confounder]{.orange}, i.e. possibly relevant for predicting the response, but not of direct interest, including it in the model may help to reduce bias in estimating relevant effects of key explanatory variables.

. . .
 
- Do not keep variables just because they are [significant]{.blue}.  
