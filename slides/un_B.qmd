...
\end{aligned}
$$
The quantity $D(\bm{y}; \hat{\bm{\mu}})/\phi$ is called [scaled deviance]{.blue} and it corresponds to a [log-likelihood ratio test]{.orange} in which the current model is tested against the saturated model.
:::
...
- $\texttt{Null deviance}$ corresponds to the [null deviance]{.blue} $D(\bm{y}; \hat{\bm{\mu}}_\text{null})$. 

- $\texttt{Residual deviance}$ corresponds to the [deviance]{.orange} $D(\bm{y}; \hat{\bm{\mu}})$ of the current model.
...
- $\texttt{Resid Df}$ are the [degrees of freedom]{.blue} of the [deviances]{.orange}, that is $n - p_0$ and $n-p$, respectively. 

- $\texttt{Resid. Dev}$ are the [deviances]{.blue} of the [reduced]{.orange} model $D(\bm{y}; \hat{\bm{\mu}}_0)$ and the [full]{.blue} model $D(\bm{y}; \hat{\bm{\mu}})$. In this example, the reduced model is also the [null model]{.orange}.  

- $\texttt{Df}$ refers to the [degrees of freedom]{.blue} $q = p- p_0$ of the test, which is $q = 1$ in this case.

- $\texttt{Deviance}$ indicates the [change in deviance]{.blue}, that is $\phi W = D(\bm{y}; \hat{\bm{\mu}}) - D(\bm{y}; \hat{\bm{\mu}}_0)$.

- $\texttt{Pr(>Chi)}$ is the [p-value]{.blue} of the log-likelihood ratio test $W$.
...
## Example: `Aids` data, output of `anova`
