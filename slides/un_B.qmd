...
- Let $Y_i \overset{\text{ind}}{\sim}\text{ED}(\mu_i, a_i(\phi)v(\mu_i))$ be the response variable of a GLM, with $g(\mu_i) = \bm{x}_i^T\beta$. The [joint distribution]{.blue} of $\bm{Y} = (Y_1,\dots,Y_n)$ is
$$
...
## Sufficient statistics and canonical link 📖
...
\end{aligned}
$$
If $\phi$ were [known]{.blue} or fixed, say $a_i(\phi) = \phi$ with $\phi$ known or $a_i(\phi) = 1/\omega_i$, then
$$
...
:::callout-warning
For now, let us focus on estimating $\beta$ and let us assume that $\phi$ is a [known parameter]{.blue}, as in binomial and Poisson regression. 

This is not a restrictive even when $\phi$ is unknown. In fact, we will show that the maximum likelihood estimate $\hat{\beta}$ does not depend on $\phi$ and that $\beta$ and $\phi$ are [orthogonal parameters]{.orange}.
:::
...
$$
which implies that the score function will have the [structure]{.orange}
$$
...
$$
- Summing up, the [chain rule of derivation]{.orange} for [composite functions]{.blue} implies:
$$
...
- Combining all the above equations, we obtain an explicit formula for the score function
$$
...
The [maximum likelihood estimator]{.blue} is the solution of [likelihood equations]{.orange}:
$$
\sum_{i=1}^n \omega_i \frac{(y_i - \mu_i)}{v(\mu_i)}\frac{x_{ir}}{g'(\mu_i)} = 0, \qquad r=1,\dots,p,
$$
which do not depend on $\phi$. In [matrix notation]{.blue}
$$
...
- When using the [canonical link]{.orange} $\theta(\mu_i) = g(\mu_i)$ there are major simplification, because
$$
\frac{\partial \theta_i}{\partial \mu_i}= \frac{1}{v(\mu_i)} = g'(\mu_i) \quad \Longrightarrow\quad v(\mu_i)g'(\mu_i) = 1.
$$
In other terms, we obtain the simple equality:
$$
\frac{\partial \theta_i}{\partial \beta_r} = x_{ir}, \qquad r=1,\dots,p,
$$
which is not surprising since the canonical link implies $\theta_i = x_{i1}\beta_1 + \cdots + x_{ip}\beta_p$.
...
$$
Let $\bm{\Omega} = \text{diag}(\omega_1,\dots,\omega_n)$, then in [matrix notation]{.blue} $\bm{X}^T\bm{\Omega}(\bm{y} - \bm{\mu}) = \bm{0}$. This simplifies even further when the weights are constant, i.e. $\bm{\Omega} = I_n$, obtaining $\bm{X}^T(\bm{y} - \bm{\mu}) = \bm{0}$.
:::
...
$$
- This system does [not]{.orange} always admits a [solution]{.orange}. This happens, for example, in the [extreme case]{.blue} $\sum_{i=1}y_i = 0$, occurring when all counts equal zero.

. . .

- Using the `Aids` data $\sum_{i=1}^ny_i = 217$ and $\sum_{i=1}^nx_i y_i = 2387$ and via [numerical methods]{.orange} we obtain $\hat{\beta}_1 = 0.304$ and $\hat{\beta}_2 = 0.259$.
...
$$
In other words, the estimated increase of Aids deaths is about the $30\%$ every trimester. 
...
- We suppose for now that $\phi$ is a [known parameter]{.orange}, say $\phi = 1$. 

- Let $\bm{I} = \mathbb{E}(\bm{J})$ be the $p \times p$ [Fisher information matrix]{.blue} associated to $\beta$, whose elements are
$$
...
- Let us first consider the negative derivative of the score function, that is
$$
\begin{aligned}
...
$$
i_{rs} = \mathbb{E}(j_{rs}) = \sum_{i=1}^n\frac{1}{a_i(\phi)}\frac{\partial\mu_i}{\partial \beta_s}\frac{\partial\theta_i}{\partial \beta_r}, \qquad \qquad r,s = 1,\dots,p.
$$

## Observed and expected information II

- Moreover, in the previous slides we already computed the derivatives $$
...
$$
The [observed information]{.blue} $\bm{J}$ is [non-stochastic]{.orange}, which means that observed information and expected (Fisher) information coincide, that is $i_{rs} = \mathbb{E}(j_{rs})$ and $\bm{I} = \bm{J}$. 
:::

. . .

:::callout-note
Under the [canonical link]{.orange}, we also have that $1/v(\mu_i) = g'(\mu_i)$, therefore we obtain
$$
i_{rs}  = \frac{1}{\phi}\sum_{i=1}^n \omega_i v(\mu_i)x_{ir} x_{is}, \qquad r,s = 1,\dots,p.
$$
In [matrix notation]{.blue}, we have that
$$
\bm{I} = \bm{X}^T\bm{W}\bm{X}, \qquad \text{ with weights} \qquad w_i = \frac{\omega_i}{\phi} v(\mu_i) = \frac{v(\mu_i)}{a_i(\phi)}, \qquad i=1,\dots,n. 
$$
:::
