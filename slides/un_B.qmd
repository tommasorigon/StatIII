...
$$
and the [hessian matrix]{.orange}, whose elements are
$$
\frac{\partial}{\partial \beta_r}\frac{\partial}{\partial \beta_s}\ell(\beta; \phi), \qquad r, s=1,\dots,p.
$$
...
$$
which does not depend on $\phi$. In [matrix notation]{.blue}
$$
...
$$
In other terms, this implies the simpler representation:
$$
\frac{\partial \theta_i}{\partial \beta_r} = x_{ir}, \qquad r=1,\dots,p,
$$
which is not surprising since in the canonical link $\theta_i = x_{i1}\beta_1 + \cdots + x_{ip}\beta_p$.
...
$$
Let $\bm{\Omega} = \text{diag}(\omega_1,\dots,\omega_n)$, then in [matrix notation]{.blue} $\bm{X}^T\bm{\Omega}(\bm{y} - \bm{\mu}) = \bm{0}$. This simplifies even further the weights are constant because $\bm{\Omega} = I_n$, obtaining $\bm{X}^T(\bm{y} - \bm{\mu}) = \bm{0}$.
:::
...
$$
- This system does not always admits a solution. This happends, for example, in the extreme case $\sum_{i=1}y_i = 0$, occurring when all counts equal zero.
