...
$$
where $\pi_i$ is the [probability]{.orange} of death at a given dose $x_i$. Moreover, et $Y_i = S_i / m_i$ be the proportion of deaths, then:
$$
...
:::aside
The Rao-score test arguably the [less common]{.orange}. When $\phi$ is unknown, there are several variants depending on how it is estimated. 
:::
...
- Let us consider a [Binomial regression]{.blue} model $m_i Y_i = S_i \sim \text{Bimonial}(m_i, \pi_i)$ with $\mu_i = \pi_i$. Then
$$
...
- Pearson residuals rescale the response residuals, accounting for [heteroschedasticity]{.orange}. 
...
- In analogy of what has been done for GLMs, we can consider the [standardized]{.blue} version of Pearson and deviance residuals.
...
Discarding observations until the hypoteshes are reasonable it skews the overall analysis and does not answer any meaningful scientific or business question. 
:::
