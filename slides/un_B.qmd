...
- When applied to real data, such as the `Aids` data, these tests often lead to very similar conclusions.

. . .

- [Wald test]{.blue} for $H_0: \beta_2 = 0$ equals `r round(lmtest::coeftest(fit_Aids)[2, 3]^2, 3)`. The Wald confidence interval is
```{r}
#| classes: styled-output
...
- [Rao test]{.orange} $W_u$ for $H_0: \beta_2 = 0$ equals `r round(anova(fit_Aids, test = "Rao")$Rao[2], 3)`. The score confidence interval is
...
- The [likelihood ratio test]{.grey} $W$ for $H_0: \beta_2 = 0$ equals `r round(anova(fit_Aids)[, 2][2], 3)`. The LRT confidence interval is
```{r}
#| classes: styled-output
