...
- GLMs are a class of [regression models]{.blue} in which a [response]{.orange} random variable $Y_i$ is modeled as a function of a vector of [covariates]{.grey} $\bm{x}_i \in \mathbb{R}^p$.  
...
$$
This is [not]{.orange} a [good approach]{.orange}. First, although $Z_i \in [0,1]$, the prediction $\beta_1 + \beta_2 x_i$ is [unrestricted]{.orange}, meaning it could produce values like "1.3" or "-2" as estimated proportions.

- Secondly, the [additive structure]{.blue} $Z_i = \beta_1 + \beta_2 x_i + \epsilon_i$ cannot hold with iid errors $\epsilon_i$, because $Y_i$, and thus $Z_i$, are [discrete]{.orange}. As a result, the errors would be [heteroskedastic]{.orange}.  
...
- The predictions are $\hat{\pi}_i = g^{-1}[\mathbb{E}\{g(\tilde{Z}_i)\}] = g^{-1}(\hat{\beta}_1 + \hat{\beta}_2 x_i)$, in which $\hat{\beta}_1$ and $\hat{\beta}_2$ are estimated using OLS. The [interpretation]{.blue} of $\hat{\beta}$ is less clear, as they refer to the mean of $g(\tilde{Z}_i)$.  
