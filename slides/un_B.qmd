...
\bm{D}^T \bm{V}^{-1}(\bm{y} - \bm{\mu}) = \bm{0}
$$
cannot be solved in closed form and we need to rely on [numerical methods]{.orange}. 
...
- There exists a huge variety of numerical methods for maximizing a function.  
...
- Instead of the maximum likelihood estimate, because of numerical instabilities and lack of robustness it is typically preferred an [method of moments]{.orange} estimator:
$$
\hat{\phi} = \frac{1}{n - p}\sum_{i=1}^n \frac{y_i - \hat{\mu}_i}{v(\hat{\mu}_i)}.
$$

- When $g(\mu_i) = \mu_i$ is the [identity link]{.blue} and $v(\mu_i) = 1$, this coincides with the usual unbiased estimator of $\sigma^2$ for a Gaussian linear model.

- For a Gamma model with $Y_i \sim \text{ED}(\mu_i, \phi \mu_i)$, we get instead
$$
\hat{\phi} = \frac{1}{n - p}\sum_{i=1}^n \frac{y_i - \hat{\mu}_i}{\hat{\mu}_i^2}.
$$
...
$$ which is the usual test statistic considered, e.g., in the output on `lm` in **R**. 

- However, in this case there is [no need]{.orange} of [approximations]{.orange}. The distribution of $z_j$ is known to be a [Student]{.grey}' $t_{n-p}$ under $H_0$, which indeed converges to a $\text{N}(0, 1)$ for large values of $n$.
...
- In this case, we [reject]{.orange} the null hypothesis that $\beta_2 = 0$. 
...
- There are [three classical tests]{.orange} that we could consider for such a testing problem: the [Wald test]{.orange}, the [Score/Rao test]{.grey} and the [log-likelihood ratio test]{.blue}. 
...
W_e \: \dot{\sim} \; \chi^2_q,
$$
namely a $\chi^2$ distribution with $q$ [degrees of freedom]{.blue}. 
:::
...
$$
is called [Rao test]{.blue} or [score test]{.blue}. Here $\widetilde{\text{var}}(\hat{\beta}_B)$ is the appropriate [block]{.grey} of $(\bm{X}\tilde{\bm{W}}\bm{X})^{-1}$ where $\tilde{\bm{W}}$ is estimated using the [restricted]{.orange} $\hat{\beta}_0$. Under $H_0$ this quantity is [approximately]{.orange} distributed as
$$
W_u \: \dot{\sim} \; \chi^2_q,
$$
namely a $\chi^2$ distribution with $q$ [degrees of freedom]{.blue}.
...
:::aside
The Rao test arguably the [less common]{.orange}. However, it is linked to the $X^2$ Pearson statistic. When $\phi$ is unknown, there are several variants depending on how it is estimated. 
:::
...
- The Wald test, the Score test and the log-likelihood ration test are [asymptotically equivalent]{.blue}. In other words, these tests give the same [same number]{.blue} for (very) large values of $n$. 
...
- The [Wald test]{.blue} for $H_0: \beta_2 = 0$ with $q = 1$ degrees of freedom equals $W_e = 135.6$, so that the p-value is $\texttt{Pr(>Chi)} = \mathbb{P}(W_e > 135.6) \approx 0$, [rejecting the null hypothesis]{.orange}. 

- Note that we previously obtained $z_j = 11.645$ and indeed $z_j^2 = 11.645^2 = 135.6$. The Wald [confidence interval]{.blue} is obtained using the analytical formulas and equals
```{r}
#| classes: styled-output
...
- The [Rao/Score test]{.orange} for $H_0: \beta_2 = 0$ equals $W_u = 163.59$ with $q = 1$ and the p-value is $\mathbb{P}(W_u > 163.59) \approx 0$. The score [confidence interval]{.blue} is obtained numerically and equals
...
- The [log-likelihood ratio test]{.grey} for $H_0: \beta_2 = 0$ equals $W = 178.55$ with $q = 1$ and the p-value is $\mathbb{P}(W > 178.55) \approx 0$. The [confidence interval]{.blue} is obtained numerically and equals
```{r}
#| classes: styled-output
...
- Suppose we were interested in a confidence interval for $\exp(\beta_2)$. Thanks to invariance of the score and the LRT tests, we can simply [transform]{.orange} of the [original interval]{.blue}:
...
- In the Wald case, we need to [adjust]{.orange} the [variance]{.orange} according to the [delta method]{.blue}, obtaining
$$\widehat{\text{var}}(\exp(\hat{\beta}_2)) = \exp(2 \hat{\beta}_2) \text{var}(\hat{\beta}_2) = 0.000830.$$
