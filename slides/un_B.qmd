...
W_e \: \dot{\sim} \; \chi^2_q,
$$
a $\chi^2$ distribution with $q$ [degrees of freedom]{.blue}. The [p-value]{.blue} is $\texttt{Pr(>Chisq)} = \mathbb{P}(W_e > w_\text{e, obs})$.
:::
...
W \: \dot{\sim} \; \chi^2_q,
$$
a $\chi^2$ distribution with $q$ [degrees of freedom]{.blue}. The [p-value]{.blue} is $\texttt{Pr(>Chisq)} =\mathbb{P}(W > w_\text{obs})$.
:::
...
W_u \: \dot{\sim} \; \chi^2_q,
$$
a $\chi^2$ distribution with $q$ [degrees of freedom]{.blue}. The [p-value]{.blue} is $\texttt{Pr(>Chisq)} =\mathbb{P}(W_u > w_\text{u, obs})$.
:::
...
| Test for the hypothesis $H_0 : \beta_2 = 0$ | $\texttt{Chisq}$ |$\texttt{Df}$ | $\texttt{Pr(>Chisq)}$|
|:---------------------------------|------:|------:|------:|
...
- For the Rao-score and the log-likelihood ratio we need numerical procedures, such as the `uniroot` function in **R**. In practice, we can use the `confint` function. 
...
| $W$ - [Log-likelihood ratio test]{.grey} | 163.586|  1|      $\approx$ 0|
...
- By construction, these confidence intervals are [positive]{.orange}, which is desirable because they represent a percentage. 
...
:::callout-tip
Whenever there are restrictions on the parametric space, as in this case, Wald is typically [problematic]{.orange}. Here, it could lead to [negative values]{.orange}, which is absurd. 
:::
