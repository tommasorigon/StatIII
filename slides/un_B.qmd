...
- The [additive structure]{.blue} $Z_i = \beta_1 + \beta_2 x_i + \epsilon_i$ cannot hold with [iid]{.blue} errors $\epsilon_i$, because $Y_i$, and thus $Z_i$, are [discrete]{.orange}. As a result, the errors are always [heteroschedastic]{.orange}. 
...
$$
\hat{\pi}_i = g^{-1}[\mathbb{E}\{g(\tilde{Z}_i)\}] = g^{-1}(\hat{\beta}_1 + \hat{\beta}_2 x_i) = \frac{\exp(\hat{\beta}_1 + \hat{\beta}_2 x_i)}{1 + \exp(\hat{\beta}_1 + \hat{\beta}_2 x_i)},$$ 
...
- This approach is [not compatible]{.orange} with the reasonable assumption $Y_i \sim \text{Binomial}(m_i, \pi_i)$.
...
:::callout-tip
We consider the [variance-stabilizing]{.blue} transformation $Z_i = \sqrt{Y_i}$, obtaining  
$$
...
- [Link function]{.grey}. This is an invertible function $g(\cdot)$ applied to each component of the [mean]{.grey} $\mu_i = \mathbb{E}(Y_i)$ that relates it to the linear predictor:
$$
...
:::callout-tip
In a GLMs the observations are independent draws from a distribution $F(\cdot \, ; \mu_i)$:
$$
Y_i \overset{\text{ind}}{\sim} F(\cdot\,;\mu_i), \qquad \mathbb{E}(Y_i) = \mu_i, \qquad g(\mu_i) = \bm{x}_i^T\beta. 
$$
...
:::{.callout-tip}
In a [Gaussian linear model]{.blue}, we have that 
$$
Y_i \overset{\text{ind}}{\sim}\text{N}(\bm{x}_i^T\beta, \sigma^2), \qquad \mathbb{E}(Y_i) = \mu_i, \qquad \mu_i = \bm{x}_i^T\beta.
$$
In this case we say that $g(\mu) = \mu$ is the [identity link]{.blue}. The variance $\sigma^2$ is an unknown parameter called [dispersion]{.orange} in GLMs, but it does not appear in the linear predictor.
:::

:::{.callout-tip}
In a [logistic regression model]{.blue}, we have that 
$$
Y_i \overset{\text{ind}}{\sim}\text{Binomial}(m_i, \pi_i), \qquad \mathbb{E}\left(Y_i\right) = \mu_i, \qquad \mu_i = \bm{x}_i^T\beta.
$$
In this case we say that $g(\mu) = \mu$ is the [identity link]{.blue}. The variance $\sigma^2$ is an unknown parameter called [dispersion]{.orange} in GLMs, but it does not appear in the linear predictor.
:::
