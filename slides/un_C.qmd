...
- The [relative risk]{.orange}, a standardized difference
  $$
  (\text{relative risk}) = \frac{\hat{\pi}(\bm{x}') -\hat{\pi}(\bm{x})}{\hat{\pi}(\bm{x})} = \frac{\hat{\pi}(\bm{x}')}{\hat{\pi}(\bm{x})} - 1.
  $$
...
## Odds ratio and logistic regression I

- The [odd]{.blue} is another way of summarizing probabilities, familiar to those who gamble:
$$
\widehat{\text{odd}}(\bm{x}) = \frac{\hat{\pi}(\bm{x})}{1-\hat{\pi}(\bm{x})}, \qquad \widehat{\text{odd}}(\bm{x}') = \frac{\hat{\pi}(\bm{x}')}{1-\hat{\pi}(\bm{x}')}.
$$
An odd represents the money you get if you win the game, for each unit you bet.

- The [odds ratio]{.orange} is another popular risk measure for comparing the two probabilities$$
(\text{odds ratio})  = \frac{\widehat{\text{odd}}(\bm{x}') }{\widehat{\text{odd}}(\bm{x}) }= \frac{\hat{\pi}(\bm{x}')}{\hat{\pi}(\bm{x})} \cdot \frac{1-\hat{\pi}(\bm{x})}{1-\hat{\pi}(\bm{x}')}.
$$

- The odds ratio is [less intuitive]{.orange} than other risk measures, but it is tightly related to logistic regression. 
...
## Odds ratio and logistic regression II

- Let us consider a covariate $\bm{x}$ and let us compute the associated odd, in a logistic regression model:
$$
\widehat{\text{odd}}(\bm{x}) = \frac{\hat{\pi}(\bm{x})}{1-\hat{\pi}(\bm{x})} = \frac{\exp(\bm{x}^T\hat{\beta})}{1 + \exp(\bm{x}^T\hat{\beta})} \cdot \left(\frac{1}{1 + \exp(\bm{x}^T\hat{\beta})}\right)^{-1} = \exp(\bm{x}^T\hat{\beta}).
$$
Thus, the odd of any predicted value is simply the exponential of the linear predictor $\bm{x}^T\hat{\beta}$. 

- Consequently, in general the [odds ratio]{.blue} is
$$
(\text{odds ratio}) = \frac{\widehat{\text{odd}}(\bm{x}') }{\widehat{\text{odd}}(\bm{x}) } = \exp\{(\bm{x}'- \bm{x})^T\hat{\beta} \}
$$
...
## Residuals and diagonistics for binary data
