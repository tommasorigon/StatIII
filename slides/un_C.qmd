...
$$
The odds ratio is [less intuitive]{.orange} than other risk measures, but it is related to logistic regression. 
...
- Let $\bm{x}$ be a generic covariate vector and consider its odd under a [logistic regression]{.blue} model:
$$
...
- In binomial regression with $m_i > 1$ the main assumption is that $S_i \sim \text{Binomial}(m_i, \pi_i)$, implying that
$$
