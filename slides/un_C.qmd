...
- We wish to [compare two estimated probabilities]{.blue}, corresponding to two different covariate vectors $\bm{x} = (x_1,\dots,x_p)$ and $\bm{z} = (z_1,\dots,z_p)$, namely
$$
\hat{\pi}(\bm{x}) = F(\bm{x}^T\hat{\beta})\qquad \text{and} \qquad \hat{\pi}(\bm{z}) = F(\bm{z}^T\hat{\beta}).
$$
...
- The [absolute risk]{.blue}, namely the difference 
  $$
  (\text{absolute risk}) = \hat{\pi}(\bm{x}) - \hat{\pi}(\bm{z}).
  $$

- The [relative risk]{.orange}, a standardized difference
  $$
  (\text{relative risk}) = \frac{\hat{\pi}(\bm{x}) -\hat{\pi}(\bm{z})}{\hat{\pi}(\bm{z})} = \frac{\hat{\pi}(\bm{x})}{\hat{\pi}(\bm{z})} - 1.
  $$

- Both these indicators are quite interpretable but depend on the specific values of $\bm{z}$ and $\bm{x}$, which makes it difficult to summarize the effect of a given covariate in a single number.
...
$$
\widehat{\text{odd}}(\bm{x}) = \frac{\hat{\pi}(\bm{x})}{1-\hat{\pi}(\bm{x})}, \qquad \widehat{\text{odd}}(\bm{z}) = \frac{\hat{\pi}(\bm{z})}{1-\hat{\pi}(\bm{z})}.
$$
An odd represents the money you get if you win the game, for each unit you bet.

  - the [odds]{.orange} $\hat{\pi}(\bm{x}) / (1-\hat{\pi}(\bm{x}))$ and $\hat{\pi}(\bm{z}) / (1-\hat{\pi}(\bm{z}))$;
  - the [odds ratio]{.blue} $\frac{\hat{\pi}(\bm{x}) / (1-\hat{\pi}(\bm{x}))}{\hat{\pi}(\bm{z}) / (1-\hat{\pi}(\bm{z}))}$.
