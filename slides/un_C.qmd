...
- When $\pi(\bm{x}) = 0.75$ then $\text{odds}(\bm{x}) = 0.75/0.25 = 3$; a success is three times as likely as a failure. In betting terms, we pay 3 to get 1 if we win. If instead $\text{odds}(\bm{x}) = 1 / 3$, we pay 1 to get 3. 
