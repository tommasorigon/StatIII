...
:::callout-note
The [likelihood]{.blue} function of these two representation [coincide]{.blue} up to a proportionality constant, which means that the maximum likelihood $\hat{\beta}$ and the standard errors are identical.
...
$$
\widehat{\text{var}}(\hat{\beta}) = (\bm{X}^T \hat{\bm{W}} \bm{X})^{-1} = (\bm{X}^T \text{diag}[m_1 \pi_1 (1-\pi_1),\dots,m_n \pi_n (1-\pi_n)] \bm{X})^{-1}
$$
...
- The [absolute risk]{.blue}, namely the difference 
  $$
  (\text{absolute risk}) = \pi(\bm{x}') - \pi(\bm{x}).
  $$
...
$$
Thus, the odds of any predicted value is simply the exponential of the linear predictor $\bm{x}^T\beta$. 
...
$$
In particular, $\exp(\beta_j)$ represents the odds ratio after a [unitary increase]{.orange} of the $j$th covariate. 
...
- For example, the `fitted` vs `residuals` plot, namely the dispersion plot of the points $(\pi_i, y_i - \pi_i)$, simply shows two parallel lines with slope $-1$, which is uninformative. 


- As an extreme case, consider the [null model]{.blue} in which $\pi_1 = \dots = \pi_n$, then the $X^2$ statistic is
$$
X^2 = \sum_{i=1}^n \frac{(y_i - \pi)^2}{\pi(1 - \pi)} = n.
$$
