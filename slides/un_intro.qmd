...
- As early as Fisher (1922), a more advanced non-linear model was introduced, designed to handle [proportion data]{.blue} of the form $Y_i / m$.

- Through some modeling and calculus, Fisher derived a [binomial]{.blue} model for $Y_i$, with
$$
\mathbb{E}(Y_i/m) = \pi_i 
...
- In the [probit model]{.blue}, developed by Bliss (1935), a [Binomial]{.blue} model for $Y_i$ is specified with
$$
\mathbb{E}(Y_i/m) = \pi_i = \Phi(\beta_1 + \beta_2 x_i),\qquad i = 1, \dots, n.
$$
...
$$
\mathbb{E}(Y_i/m) = \pi_i = \frac{\exp(\beta_1 + \beta_2 x_i)}{1 + \exp(\beta_1 + \beta_2 x_i)}, \qquad i = 1, \dots, n.
$$
