...
**Nota**: un modello di regressione lineare semplice, applicato direttamente alle proporzioni $Y_i$ è un approccio errato e corrisponde al "*linear probability model*" menzionato in classe, che è molto problematico per tutte le ragioni discusse a lezione e nel materiale online.
...
$$
\hat{\mathbb{E}}(Y_i) = \hat{\pi}_i = \frac{\exp^{-6.01 + 0.3412 x_i}}{1 + \exp^{-6.01 + 0.3412 x_i}}, \qquad i=1,\dots,10.
$$
```{r}
#| results: hide
# Punto (b.ii)
...
Il valore delle statistiche test per i test di Wald e log-rapporto di verosimiglianza sono, rispettivamente: $W_e = 67.522$ (Wald) e $W = 100.3 / \phi = 100.3$ (log-rapporto di verosimiglianza), poichè $\phi = 1$ nel modello binomiale. Nel caso del test di Wald, era anche corretto riportare la statistica test `z-value`, pari a $8.217$, il cui quadrato coincide con $W_e$ a meno di approssimazioni numeriche.
...
Considerati i gradi di libertà dei test $W_e$ e $W$ e la loro distribuzione asintotica sotto $H_0$, si ottiene in entrambi i casi un *p-value* praticamente pari a $0$. In altri termini si rigetta l'ipotesi nulla che $\beta_2 = 0$. Questo significa che la probabilità di decesso varia con la concentrazione di etilene (`conc`). 
...
La 1a osservazione e l'8a osservazione hanno una distanza di Cooks elevata, superiore a $1$, mentre la sesta osservazione è *borderline* (ulteriori comandi mostrano che la 6a osservazione ha residuo molto alto ma valore leva relativamente basso). Valori alti della distanza di Cooks identificano appunto osservazioni influenti, cioè al contempo punti leva e potenzialmente anomale. 

**Nota**: alcuni studenti hanno fatto uso di strumenti grafici diagnostici, come ad esempio `plot(m1, which = 4)`, oppure si sono affidati a librerie esterne, come il comando `influencePlot` della libreria `car`. Si tratta di soluzioni corrette, per cui non ho dato penalizzazioni, tuttavia in nel caso del comando `InfluencePlot` viene fatto uso dei cosiddetti **residui studentizzati**, la cui definizione non è mai stata discussa a lezione nel caso dei GLM e non è presente in nessuno dei libri di testo. Ciò che la funzione `rstudent` sta calcolando nel caso dei GLM è la seguente "media pesata" tra residui di Pearson e residui di devianza standardizzati:
$$
...
Il valore $\exp(\beta_2) = 1.4067$ si può interpretare come rapporto tra quote relativo (*odds ratio*) relativo all'aumentare unitario della concentrazione di etilene. Tale rapporto è positivo e abbastanza elevato, ad indicare un marcato aumento della probabilità di decesso al crescere della variabile `conc`.

```{r}
# Punto (f)
new15 <- data.frame(conc = 15)
predict(m1, newdata = new15, type = "response") # Probabilità prevista
```
...
round(c(X2, X2 / m1$df.residual, 1 - pchisq(X2, m1$df.residual)), 4)
```

È senz'altro presente sovradispersione. Il valore di $X^2$ è elevato e al di fuori dei valori tipici, come confermato dal *p-value* associato. Si ottiene inoltre che $\hat{\phi} = X^2 / (n - p) = 4.432$.
...
Lo stimatore di quasi-verosimiglianza produce le medesime stime puntuali, ma corregge gli errori standard. Fortunatamente, questo non altera le conclusioni inferenziali a cui eravamo giunti in precedenza. Tuttavia, si noti che le osservazioni che in precedenza erano state registrate come "anomale" ed "influenti", rientrano nella norma una volta che si tiene conto della sovradispersione:
...
Anche in questo caso, è possibile specificare due intervalli di confidenza alternativi (entrambi corretti). Confrontati con quelli ottenuti in precedenza, si nota che la loro ampiezza è aumentata significativamente come conseguenza della sovradispersione.