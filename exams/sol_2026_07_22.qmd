...
$$
\frac{\mu_{jk}}{\mu_{kj}} = \exp\left\{\alpha + \beta_j + \gamma_k - (\alpha + \beta_k + \gamma_j)\right\} = \exp\left\{(\beta_j - \gamma_j) - (\beta_k - \gamma_k)\right\} = \exp(\delta_j - \delta_k).
$$
...
Intervallo di [previsione]{.blue} (non di [confidenza]{.orange}!) per i gol di Francia è quindi $[0, 4]$, un intervallo molto ampio. 

::: {.column-margin}
Per un refuso, il punto (d.ii) chiedeva l'intervallo di previsione per il numero di gol segnati dalla Francia contro la **Spagna**, anziché contro l'**Argentina** come nel resto del punto (d). La soluzione qui riportata si riferisce a Francia–Argentina; le due previsioni risultano comunque coincidenti, per cui nessuna delle due letture comporta differenze nel risultato.
:::
