...
$$
\hat{\mathbb{E}}(Y_i) = \hat{\pi}_i =\frac{e^{\hat{\beta}_1 +  \hat{\beta}_2 d_i}}{1 + e^{\hat{\beta}_1 +  \hat{\beta}_2 d_i}} =\frac{e^{-3.504 +  0.405 d_i}}{1 + e^{-3.504 + 0.341 d_i}}, \qquad i=1,\dots,10000,
$$
...
È quindi chiaro che la probabilità di insolvenza di uno studente è maggiore di quella di un cliente non studente. In alternativa, si sarebbero potute considerare le quote.
...
Il cui rapporto (*odds ratio*) è appunto pari al valore `1.499` calcolato prima. 
...
```
  ...
  - Il valore $e^{\hat{\beta}_2} = 0.524$, relativo alla variabile `student`, è interpretabile come rapporto tra quote ed è minore di $1$. Si noti inoltre che $e^{\hat{\beta}_2}$ è significativamente minore di $1$, come si desume dal *p-value* associato. Questo significa che, a parità di debito e reddito (`balance` ed `income`), essere studente è associato a una probabilità di insolvenza [minore]{.orange} rispetto ad un non-studente.  Questa conclusione è solo all'apparenza in contraddizione con quanto illustrato dal modello `m1` ed è un esempio del cosiddetto [paradosso di Simpson]{.blue}, nel quale l'aggiunta di una covariata modifica la direzione o l'entità di un'associazione marginale. In altri termini, non è tanto la condizione di studente in quanto tale a comportare un rischio di insolvenza, quanto piuttosto il debito accumulato nella carta di credito, che a sua volta è, in media, maggiore tra gli studenti.

- Il valore $e^{\hat{\beta}_4} = 1.006$, relativo alla variabile `student`, è interpretabile come rapporto tra quote ed è maggiore di $1$. In altre parole, all'aumento unitario ($1\$$) della variabile `balance` la quota dell'insolvenza cresce dello $0.6\%$. Considerata la scala della variabile di riferimento, questo è un aumento piuttosto importante. Per esempio, un aumento del debito medio di $500\$$ comporta un rapporto tra quote pari a $e^{500 \hat{\beta}_4} = 17.606$, cioè un aumento della quota di insolvenza del $1660.6\%$.
