...
p.value<-1-pchisq(mod.pois$deviance,24);p.value
...
phi.hat<-sum(residuals(mod.pois,type="pearson")^2)/df.residual(mod.pois);phi.hat
...
phi.hat2<-sum(residuals(mod.pois2,type="pearson")^2)/df.residual(mod.pois2);phi.hat2
...
phi.hat<-sum(residuals(mod.rate2,type="pearson")^2)/df.residual(mod.rate2);phi.hat
...
#previsione
new<-data.frame(AgeInd=26,ER=95,
